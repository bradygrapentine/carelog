...
|---|---|---|
| 🟢 Ready | 21 | TD-111 · TD-133 · TD-141 · TD-142 · TD-143 · TD-144 · UX-053 · UX-065 · UX-077 · UX-103b/104b/105b · SEC-008 · SEC-009 · SEO-005 · PP-009 · ON-70 · ON-71 · ON-74 (+ rows in §3–§5 not enumerated) |
| 🔎 In review | 0 | — |
...
| TD-143 | 🟢 Ready · **P3 / Low** | **Mood-label map duplication — extract shared `MOOD_LABELS` constant + reconcile `okay: "Settled"` divergence** | Surfaced 2026-05-15 by `/oop --from-sprint` review of UX-050 sprint. After the `"Crisis"` → `"Hard"` rename, 6 mood-label maps live in the codebase: `JournalEntryForm.tsx`, `SymptomPanel.tsx`, `MoodSpectrum.tsx`, `PromptedComposer.tsx`, `RecipientProfile.tsx`, `server/routers/moodEntries.ts`. Five maps share `{good: "Good", okay: "Okay", difficult: "Difficult", crisis: "Hard"}`. The sixth (`moodEntries.ts:17`) has `okay: "Settled"` — the only divergent label. Undocumented whether intentional (server-side digest tone variation) or accidental drift. **Decision needed:** (a) extract a single `MOOD_LABELS` const in `apps/web/lib/mood.ts` (already houses the `Mood` type), make moodEntries.ts override `okay` explicitly with a comment explaining the digest-tone choice (or revert to "Okay"); (b) leave divergence as-is, add an inline comment at `moodEntries.ts:17` explaining the rationale. Path (a) eliminates the byte-identical triple (JournalEntryForm/SymptomPanel/PromptedComposer arrays) while preserving the digest-tone affordance. **Risk:** LOW. **Size:** S (~30min refactor + tests). |
