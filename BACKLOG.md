...
|---|---|---|
| 🟢 Ready | 19 | TD-111 · TD-133 · TD-141 · TD-142 · TD-144 · TD-146 · UX-065 · UX-077 · UX-103b/105b · SEC-008 · SEC-009 · SEO-005 · PP-009 · ON-70 · ON-71 · ON-74 (+ rows in §3–§5 not enumerated) |
| 🔎 In review | 0 | — |
...
| TD-163 | 🟢 Ready · **P3 / Low** | — | — | **Delete stale screenshot artifact under `QuickLogFab.test.tsx/__screenshots__/`** | Surfaced 2026-05-17 by `/oop --from-sprint` review. `apps/web/components/__tests__/__screenshots__/QuickLogFab.test.tsx/-QuickLogFab----navigates-to-dashboard-when-no-recipientId-in-URL-1.png` is an orphan PNG — the test it covers was dropped in PR #552 (TD-150 fix). Delete; if vitest's screenshot harness needs regeneration, run the screenshot snapshot script. **Risk:** LOW. **Size:** XS (~5 min). |
| TD-164 | 🟢 Ready · **P3 / Low** | — | — | **Validate `recipientId` from pathname as UUID before `router.push` in QuickLogFab** | Surfaced 2026-05-17 by `/oop --from-sprint` review of observability-followups sprint. `apps/web/components/QuickLogFab.tsx:72-73` extracts `recipientId` via `pathname.match(/^\/journal\/([^/?]+)/)`. The capture group permits `..` and URL-encoded chars; the value flows directly into `router.push(\`/journal/${recipientId}?panel=...\`)`. Low risk today (router.push is client-controlled, destination is internal Next.js route), but defense-in-depth dictates UUID validation before navigation — consistent with the UUID type on tRPC inputs elsewhere. **Fix:** check via existing UUID regex or zod before navigation; bail with a Sentry breadcrumb if invalid. **Risk:** LOW. **Size:** XS (~15 min). |
| TD-165 | 🟢 Ready · **P3 / Low** | — | — | **Wrap care_events Promise.all in try/catch with finally for network-failure path** | Surfaced 2026-05-17 by `/oop --from-sprint`. TD-152 captures Supabase data-errors via `result.error` inspection (correct — Supabase JS never throws on query errors). But an actual **network** failure (DNS down, fetch rejection) WILL throw and unwind the async IIFE in `apps/web/app/(app)/dashboard/DashboardClient.tsx:67-201`, leaving `loading=true` and skeletons rendered forever. Complement TD-152 by wrapping the `Promise.all` block in try/catch with `setLoading(false)` in finally, mirroring the existing pattern at lines 88-115 (Stripe pendingPlan bridge). Sentry.captureException in the catch block with `tags: { component: "DashboardClient", path: "network" }`. **Risk:** LOW. **Size:** S (~30 min — small refactor + targeted vitest). |
