...
| ID | Status | Story | Notes |
|---|---|---|---|
| ON-75 | 🟢 Ready · **P0** | **Onboarding wizard — care team setup fails: RLS blocks `organizations` insert** | Root cause confirmed 2026-05-10. `POST /api/onboarding/create` returns 500 with `Error: Org creation failed: new row violates row-level security policy for table "organizations"` thrown from `server/repositories/organizationsRepository.ts:34`. The `organizations` INSERT policy must require something the just-signed-up user does not yet have (likely a `memberships` row that is created later in the same flow — chicken-and-egg). Fix path: either (a) loosen the INSERT policy to allow any authenticated user to insert with `created_by = auth.uid()`, OR (b) move the org+membership creation behind a SECURITY DEFINER function so both rows land atomically. Add pgTAP coverage. **Blocks every signup in production** — treat as immediate P0. |
