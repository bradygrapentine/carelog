...
| UX-040 | 🟡 Spike | **BriefHero empty-state model — derive vs. require generated brief** | `/impeccable critique` 2026-04-29 raised: BriefHero only populates when a `care_briefs.headline` row exists. New users and any day before someone manually triggers generation see an empty hand-off surface. PRODUCT.md says the product should "know what an exhausted Tuesday at 11pm looks like" — asking the caregiver to generate a brief first is the opposite of that. Decision needed: should the dashboard derive its own always-present summary from raw `care_events` when no brief exists, or stay with the explicit-generation model? Founder/PM decision before any code. |
