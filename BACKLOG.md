...
Last consolidated: **2026-04-16** (codebase scan same day). Last `/backlog-sync`: **2026-05-15 PM (ON-76 close + TD-146 seed)** — `/sprint copy-audit-trio-finish` shipped UX-049 + UX-050 (PRs #528/#529). Auth/onboarding voice pass + journal mood/entry-form copy + `"Crisis"` → `"Hard"` mood-label rename across 6 sites. 2 /opus-on-opus cycles before clear (4MF/3SF/2N → 0/0/0); cycle-1 caught OnboardingForm consolidation already shipped (NO-OP), moodEntries.ts orphan, PromptedComposer regex matchers. /oop --from-sprint clean (0/1/2) — surfaced TD-143 candidate (moodEntries.ts:17 `okay: "Settled"` divergent from chip label "Okay"). §0 Ready: 22 → 21 (-2 shipped + 1 new TD-143).
