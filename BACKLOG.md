...
|---|---|---|
| 🟢 Ready | 16 | TD-111 · TD-133 · TD-141 · TD-142 · TD-144 · TD-166 · UX-065 · UX-077 · UX-103b/105b · SEC-008 · SEC-009 · SEO-005 · PP-009 · ON-70 · ON-71 · ON-74 (+ rows in §3–§5 not enumerated) |
| 🔎 In review | 0 | — |
...
| TD-154 | 🟢 Ready · **P3 / Low** | — | — | **OTP error wording conflates "invalid" with "expired"** | Surfaced 2026-05-17 by Cowork auth-unhappy suite run `1778990299`. Frontend renders `"The code expired. Send a new one."` for both a known-invalid code (`000000`) and a genuinely-expired code — Supabase `/auth/v1/verify` returns 403 for both, and the UI doesn't distinguish. **Security:** good (no oracle that differentiates wrong vs expired). **UX:** bad — a user who mistypes is told the code expired and prompted to request a fresh one, which invalidates their actually-still-valid current code. Self-reinforcing failure loop. **Fix options:** (a) display `"Invalid code"` in both cases (still no oracle, more accurate); (b) distinguish server-side via response body and surface separate UX messages. **Risk:** LOW. **Size:** XS (~30 min — wording change in auth callback handler). |
...
| TD-159 | 🟢 Ready · **P3 / Low** | — | — | **"Export care history (coordinators only)" label visible to non-coordinators** | Surfaced 2026-05-17 by Cowork RLS suite run `1778990299` scenario 5. The Settings → Danger zone entry "Export care history" carries the "(coordinators only)" sub-label, visible to ALL users regardless of role. The link itself is presumably gated server-side; this is about whether the *capability disclosure* is intentional. **Decision needed:** (a) hide the entire row for non-coordinators (cleaner — capability stays invisible); (b) keep visible as a soft upgrade nudge if that's the product intent. Cowork couldn't probe with an actual non-coordinator user (both test accounts are coordinators), so confirm the runtime behavior first. **Risk:** LOW. **Size:** XS (~15 min UI fix). |
...
| TD-166 | 🟢 Ready · **P3 / Low** | — | — | **`setLoading(false)` unreachable on `pending_invite` + `pendingPlan` redirect paths in DashboardClient** | Surfaced 2026-05-17 by `/oop --from-sprint` review of td-146-followups sprint. Pre-existing on main, NOT regressed by TD-165. The early `return` at `apps/web/app/(app)/dashboard/DashboardClient.tsx:80` (`pending_invite` bridge → `/invite/TOKEN`) and `:111` (`pendingPlan` bridge → Stripe Checkout URL) sit before the TD-165 `try` block. If the navigation away is slow or the user presses Back, the skeleton renders forever. **Fix:** hoist `setLoading(false)` into a wrapper try/finally enclosing the entire IIFE body, OR call it explicitly before each early `return`. Also add a vitest case for the `pending_invite` + back-navigation path. **Risk:** LOW (edge case — most users do navigate away successfully). **Size:** XS (~15 min). |
