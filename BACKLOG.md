...
Last consolidated: **2026-04-16** (codebase scan same day). Last `/backlog-sync`: **2026-05-14 (post oop-wave-a re-audit)** — `/oop` Phase 2 re-audit against current main invalidated two prior OOP rows: OOP-003 struck (`messaging_rls.test.sql` already exists on main with `plan(12)`, 13 assertions) and OOP-007 struck (`user_is_org_coordinator()` already exists + already widely consumed; 0 inline `EXISTS-from-memberships` patterns remain). OOP-010 redirected from AsyncStorage to `expo-secure-store` (project uses SecureStore, not AsyncStorage). Fresh audit doc at `docs/plans/2026-05-14-oop-refactor.md`; stale predecessor archived at `…-oop-refactor.STALE.md`. Remaining Ready = 38.
...
|---|---|---|
| 🟢 Ready | 38 | TD-111 · TD-120..123 · **TD-129/130** · UX-041..044 · UX-048..050 · UX-053 · UX-065 · UX-077 · UX-103b/104b/105b · SEO-005 · PP-009 · ON-70 · ON-71 · ON-74 · **OOP-004/006/008..015** (+ rows in §3–§5 not enumerated) |
| 🔎 In review | 0 | — |
...
| OOP-009 | 🟢 Ready | **Mobile `Offline Queue` mutation routing via `PayloadMutator`** | `apps/mobile/hooks/useOfflineWrite.ts:15-71` — inline `MutationMap` projects `write.payload` and builds tRPC args by hand for 3 entry kinds. Brittle to payload-shape changes. Extract `PayloadMutator` interface + `JournalMutator`, `MedicationMutator`, `SymptomMutator` under `apps/mobile/lib/offlineMutators/`. Hook composes via registry. Retry/idempotency contract unchanged. **Risk:** LOW. **Size:** M (3–4h). Plan doc §Wave C. |
| OOP-010 | 🟢 Ready | **Mobile `SecureStore` key registry** | Redirected 2026-05-14 re-audit: project uses `expo-secure-store`, not AsyncStorage. Verified hardcoded keys `"pending_email"`/`"pending_invite_token"` (string literals in `(auth)/sign-in.tsx`, `(auth)/verify.tsx`, `(app)/invite/[token].tsx`) plus module-level constants `ONBOARDING_KEY` (`lib/onboarding.ts`), `QUEUE_KEY` (`store/offlineQueue.ts`). 5 files own their own key strings with no namespacing/versioning. New: `apps/mobile/lib/secureStoreKeys.ts` exporting `KEYS` with `v1:carelog:…` prefix. Migration path: read fallback to old keys, write back under new. **Risk:** LOW. **Size:** S (1h). Plan doc §Wave C. |
| OOP-011 | 🟢 Ready | **Mobile tRPC `useMutationWithRefresh` composable** | Every screen hand-rolls `useQuery + useMutation({onSuccess: refetch})` — `app/(app)/journal/index.tsx:45-51`, `journal/[eventId].tsx`, `outer-circle/index.tsx:45-62`, +~15 more. Collapse to per-router hooks (e.g. `useJournalEntries()`) or one generic `useMutationWithRefresh`. Loading/error parity preserved. **Risk:** LOW. **Size:** S (1–2h). Plan doc §Wave C. |
| OOP-012 | 🟢 Ready | **`HandoffSummary` name hydration via `NameResolver` injection** | `apps/web/lib/handoffSummary.ts:99-104` — callers must construct an `actorNameMap` before calling; incomplete maps silently fall back to "Team member" (L76). New: `NameResolver` interface (`getName(id) → string \| null`) injected into `buildHandoffSummary()`. `CachedNameResolver` reference impl. Output identical. **Risk:** LOW. **Size:** S (1–2h). Plan doc §Wave C. |
| OOP-013 | 🟢 Ready | **Mobile push notification `NotificationRouter` registry** | `apps/mobile/app/_layout.tsx:68-82` — hardcoded `if (data?.screen === "ocr-review" && data?.jobId)` only handles OCR. Future screen types add an if. Push payload `{jobId?, screen?}` shape inferred, not typed. Define `NotificationPayload` Zod schema; `NotificationRouter` interface; per-screen routers (OCR + a placeholder for messaging); root dispatcher loop. **Risk:** LOW. **Size:** M (2–3h). Plan doc §Wave C. |
| OOP-014 | 🟢 Ready | **Mobile `useSyncStatus` singleton + AppState-aware polling** | `apps/mobile/hooks/useSyncStatus.ts:18-23` — independent 2-s `setInterval` per consumer (3 screens spin 3 timers). Wakes device every 2s when backgrounded. Refactor to singleton `SyncStatusManager` context with one timer + AppState listener that pauses on background. Foreground values + cadence unchanged. **Risk:** LOW. **Size:** M (2–3h). Plan doc §Wave C. |
| OOP-015 | 🟢 Ready | **Stripe webhook switch → handler-per-event map + Sentry error route** | `apps/web/app/api/stripe/webhook/route.ts:22-120` — single switch for 4 event types; adding a new type means editing the route. Companion fix: error path currently sinks exceptions into PostHog `$exception` with `"anonymous"` distinctId — route to `Sentry.captureException` with `{event_type}` tag (no PII). New: `handlers: Record<string, (event) => Promise<void>>` map; per-event handler module under `app/api/stripe/webhook/handlers/`. **Risk:** LOW. **Size:** S–M (1.5–3h). Plan doc §Wave C / cross. |
