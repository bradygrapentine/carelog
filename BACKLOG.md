...
|---|---|---|
| 🟢 Ready | 15 | TD-133 · TD-141 · TD-174 · TD-175 · TD-177 · TD-178 · TD-179 · TD-182 · TD-183 · TD-184 · TD-186 · UX-077 · SEC-008 · SEC-009 · PP-009 · ON-70 · ON-71c · ON-71d · ON-74 (+ rows in §3–§5 not enumerated) |
| 🔎 In review | 0 | — |
...
| TD-186 | 🟢 Ready · **P3 / Low** | — | — | **TD-180/181 /oop residuals — cancel-contract, error pass-through, Sentry.setTag coverage** | Surfaced 2026-05-18 by `/oop --from-sprint` on PRs #616+#617. Three small follow-ups: (1) `EmergencyFooterCard.tsx:85-93` — `handleCancel` resets form state AND calls `editMode.cancel()` separately; if hook's `cancel()` is called directly the form keeps stale dirty state. Either move field-reset into a `useEffect` keyed on `isEditing` edge, or document the contract in the hook's JSDoc. (2) `CareTeamList.tsx:122-127` + `EmergencyFooterCard.tsx:54-58` — `onError` discards the real tRPC error message and substitutes a generic string. Should pass through: `onError: (err) => editMode.handlers.onError(err)` so 4xx vs 5xx vs validation errors surface to users. (3) `apps/web/eslint-rules/no-phi-in-analytics.js:73` — rule doesn't cover `Sentry.setTag`/`setTags`/`setExtra`/`setExtras` (common alternates to setContext). Add to `isTargetCall` matcher or inline-comment the gap. **Risk:** LOW. **Size:** S (~1.5h — all 3 fixes + tests). |
...
| TD-183 | 🟢 Ready · **P3 / Low** | — | — | **Consolidate week-stamp helpers in `@carelog/utils`; fix `weeklyDigest` ISO compliance** | Surfaced 2026-05-17 by `/oop --from-sprint` on ON-71 Phase 2 (PR #599). Two week-stamp implementations now: the new `isoWeekStamp` in `apps/web/inngest/functions/refillAlert.ts` is ISO-8601-correct (Thursday-of-week rule, zero-pad). The existing `getWeekStamp` in `packages/utils/src/index.ts:6-14` is hand-rolled local-time math without the Thursday rule, no zero-pad — consumed by `weeklyDigest.ts`. This means `weeklyDigest` and `refillAlert` could disagree on what "week" any given date belongs to at year-end boundaries. **Fix:** move `isoWeekStamp` to `@carelog/utils`, retire the old `getWeekStamp`, sweep callers (vitest will catch the rest). Document the ISO 8601 contract. **Risk:** LOW (functions only diverge at year-edge dates; week-stamps only used for idempotency keys not display). **Size:** XS (~45 min — move + callsite sweep + 1-2 boundary tests). |
