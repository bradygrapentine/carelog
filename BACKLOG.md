...
| TD-16 | 🔎 In review | **Clear all web tsc errors + wire CI to catch web typecheck** | Added `"type-check": "tsc --noEmit"` to `apps/web/package.json` (CI's turbo `type-check` was silently skipping web). Added `"types": ["vitest/globals"]` to tsconfig (fixed ~104 TS2582 errors in test files). Fixed 26 files covering: ShiftForm strict-schema fields, MedicationLinkedDocs/RecentEvents missing `org_id` prop, SidebarNav/EducationTipWidget base-ui `asChild` incompatibility, careEventCommentsRepository Supabase FK join inference, rateLimit test `NODE_ENV` readonly cast, createCaller Context union type in 3 router tests, messagesRepository rpc mock type, ErrorBoundary test ReactNode return type, vitest-axe `export type *` mismatch suppressed with `@ts-expect-error`. Result: 0 errors, 1111/1111 tests green. |
