...
|---|---|---|
| 🟢 Ready | 13 | TD-111 · TD-133 · TD-141 · TD-144 · TD-167 · UX-065 · UX-077 · UX-103b/105b · SEC-008 · SEC-009 · SEO-005 · PP-009 · ON-70 · ON-71 · ON-74 (+ rows in §3–§5 not enumerated) |
| 🔎 In review | 0 | — |
...
| TD-167 | 🟢 Ready · **P3 / Low-Med** | — | — | **`/api/invite/[token]/accept` route echoes raw Postgres error messages to client** | Surfaced 2026-05-17 by `/oop --from-sprint` on TD-142 (PR #567) — also flagged in the TD-142 /owasp pre-plan as T8. `apps/web/app/api/invite/[token]/accept/route.ts:32` returns `error.message` directly in the JSON 500 response. Postgres / PostgREST error strings can include table names, constraint names, parameterized values, and (in edge cases) the invite token value itself. **Fix:** log `error.message` server-side (Sentry), return a generic `"Internal error"` string to the client. Mirror the same pattern in the tRPC `memberships.ts:89` accept-mutation error branch. **Risk:** LOW (existed pre-TD-142, no live exploit observed). **Size:** XS (~15 min — two error branches + tests asserting the response body is generic). |
