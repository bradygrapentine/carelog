...
|---|---|---|
| 🟢 Ready | 13 | TD-111 · TD-133 · TD-141 · TD-144 · TD-169 · UX-065 · UX-077 · UX-103b/105b · SEC-008 · SEC-009 · SEO-005 · PP-009 · ON-70 · ON-71 · ON-74 (+ rows in §3–§5 not enumerated) |
| 🔎 In review | 0 | — |
...
| TD-169 | 🟢 Ready · **P3 / Low** | — | — | **`callerError`/`targetError` branches in `changeRole` + `remove` swallow Supabase transport errors as FORBIDDEN/NOT_FOUND without Sentry capture** | Surfaced 2026-05-17 by `/oop --from-sprint` on TD-168 (PR #572). The TD-167/168 sweep covered every site that echoed a raw error to the client, but the symmetric defensive branches throw bare `TRPCError({ code: "FORBIDDEN" })` / `NOT_FOUND` when Supabase returns an `error` object — meaning a real transport failure (DB unreachable, connection refused) is indistinguishable from a legitimate auth failure in Sentry. Sites: `memberships.ts` `changeRole` ~165 (callerError), `remove` ~225 (callerError). Same pattern likely on the `targetError` branches at ~180 and ~240. **Fix:** when the early-exit branch is taken AND `<x>Error` is non-null (vs `!caller`/`!target` for legitimate not-found), `Sentry.captureException(wrapAdminError(error), { tags: { component, path: "caller.error" \| "target.error" } })` before throwing. Client message unchanged. **Risk:** LOW. **Size:** XS (~15 min — 4 branches + 2-3 vitest cases). |
