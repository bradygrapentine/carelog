...
| TD-89 | 🟢 Ready | **Extract `<FormActionRow>` — Save/Cancel form footer** | `/impeccable extract` Phase A: ≥10 inline forms repeat the `<Button type="submit" disabled={loading}>Save</Button> <Button variant="ghost" type="button" onClick={cancel}>Cancel</Button>` pattern across `app/(app)/journal/[recipientId]/` (CoverageSettings, ShiftForm, MedicationPanel, BurnoutCheckin, SymptomPanel, OuterCirclePanel, EolPlanner, JournalEntryForm, ExpensePanel) plus `components/shifts/TradeRequestForm`, `ShiftPopover`, `marketing/ContactForm`. Lift to `apps/web/components/ui/FormActionRow.tsx` with `submitLabel`, `loading`, `disabled`, optional `onCancel`. Keep separate from `<AlertDialogFooter>` (different intent). S+M effort. Candidate #5. |
| TD-90 | 🟢 Ready | **Extract `<ErrorBanner>` — block-level form-error banner + fix raw `bg-red-50` token leaks** | `/impeccable extract` Phase A: 3 form-banner sites (`OnboardingForm.tsx:134`, `signin/page.tsx:32`, `team/admin/TeamAdminClient.tsx:78`) repeat the danger-tinted box. The TeamAdmin one already uses `--color-danger-subtle` token; the older two hardcode `bg-red-50` — token-drift bonus fix. Lift to `apps/web/components/ui/ErrorBanner.tsx` wrapping `rounded-xl bg-[var(--color-danger-subtle)] px-4 py-3 text-sm text-[var(--color-danger)]` with `role="alert"`. Keep inline `<p className="text-sm text-[var(--color-danger)]">` field-error captions as-is — different intent. S+S effort. Candidate #3. |
| TD-91 | 🟢 Ready | **Widen `lib/mood.ts` — mood→class lookup helpers** | `/impeccable extract` Phase A: 5 distinct mood→color mappings across `JournalTimeline.tsx:16-19,26-29,36-41,332-337`, `PatternsStrip.tsx:18`, `SymptomPanel.tsx:66-68`. Most use `color-mix(in_oklab, ..._N%, white)` family with 4 alpha tiers (12/15/18/22%) — likely intentional (chip-resting vs chip-selected vs strip-marker), confirm before flattening. Add `moodBgClass`, `moodBorderClass`, `moodChipClass(mood, { selected })` to `lib/mood.ts`. M effort — risk of behavior drift if alpha percentages aren't preserved verbatim. Candidate #4. |
| TD-92 | 🟢 Ready | **Widen `<TintedCard>` — absorb dark-mode tokens** | `/impeccable extract` Phase A surprising finding: `<TintedCard>` already exists and is adopted in 7 sites, but `components/shifts/TradeRequestList.tsx:71` had to bypass it for dark-mode support. The right move is to *widen* the existing primitive to accept dark-mode token variants, not extract a new component. Audit all current `<TintedCard>` consumers to confirm dark-mode parity, then update `TradeRequestList.tsx:71` to use the widened API. M effort. |
| TD-93 | 🟢 Ready | **Add `lib/pdfTokens.ts` — design tokens parallel for react-pdf** | `/impeccable extract` Phase A token-gap finding: PDF export files (`apps/web/app/api/history/export/pdf/route.tsx`, `app/api/export/ExportDocument.tsx`) hold ~12 raw hex values that *cannot* consume CSS variables (react-pdf doesn't resolve them). Solution is a parallel `apps/web/lib/pdfTokens.ts` constants file mirroring the design system's color names. Source the hex from `globals.css` `@theme inline` so the two stay in sync (or document the rule that adding a new design token requires adding to `pdfTokens.ts` too). S effort. |
