...
| TD-149 | 🟢 Ready · **P1 / High** | — | — | **Investigate + fix 503 on `HEAD /care_events?select=*&org_id=…`** | Surfaced 2026-05-16 by post-SEC-001 happy-path live-test (Cowork run `1778988534`). Reproducible 503 on the unconstrained `HEAD` against `care_events` for a real `org_id`; the equivalent `GET ?select=created_at&order=created_at.asc&limit=1` returns 200 with the same filter. Likely candidates: postgres-level error triggered by the unbounded `SELECT *` HEAD (timeout, missing index on `(org_id, created_at)`, trigger error, or PostgREST quirk with HEAD on tables with certain RLS policies). **Blocks journal entries entirely on the dashboard** — Quick log fails to render anything because the page is in a degraded state from this 503. **Reproduce:** signin as a brady.grapentine test account → open dashboard → DevTools Network → filter `care_events` → first HEAD returns 503. **Investigation steps:** (1) check Supabase logs (Database → Logs) for the timestamp of the failing HEAD; (2) check `EXPLAIN` on the equivalent SELECT *; (3) check whether RLS policies on `care_events` evaluate per-row in a way that breaks HEAD semantics. **Risk:** HIGH (production journal write is dark). **Size:** S–M. |
| TD-150 | 🟢 Ready · **P1 / High** | — | — | **Quick log menu items (mood/note/medication) silently fail to open modal** | Surfaced 2026-05-16 by post-SEC-001 happy-path live-test (Cowork run `1778988534`). On the dashboard, clicking any Quick log option (Log mood, Log note, Log medication) closes the menu without opening any modal/sheet. `document.querySelectorAll('[role="dialog"]').length === 0` post-click. Zero console errors, zero new network requests, zero Sentry events. May or may not be related to TD-149 (the `care_events` HEAD 503 may be putting the dashboard in a degraded state that disables modal opening). **Investigation steps:** (1) determine whether the bug reproduces when TD-149 is fixed; (2) audit the click handlers in `apps/web/components/app/QuickLog*.tsx` (or equivalent) for early returns / silent throws; (3) confirm the modal trigger pattern (BaseUI Dialog? Radix? state-based portal?). **Risk:** HIGH (core write path is non-functional in prod). **Size:** S (likely a focused handler fix once located). |
| TD-151 | 🟢 Ready · **P2 / Medium** | — | — | **Publish DMARC TXT record on `care-log.org` + move sender off `noreply@` apex** | Surfaced 2026-05-16 by post-SEC-001 happy-path live-test (Cowork run `1778988534`). The Resend → Supabase Auth SMTP rotation works end-to-end (OTP delivered to Resend, accepted by Gmail SMTP), but Gmail **silently drops** the message: no inbox, no spam folder, no bounce. Resend Insights flags two issues on `care-log.org`: (1) no DMARC TXT record on the apex domain — Gmail tightened DMARC enforcement in 2024 and classifies DMARC-less mail from `noreply@` senders to high-entropy `+tag` recipients as enumeration/spam; (2) `noreply@care-log.org` sender flagged as reputation-degrading. **Fix:** publish `_dmarc.care-log.org. TXT "v=DMARC1; p=none; rua=mailto:dmarc@care-log.org"` (start at `p=none` to monitor before tightening to `quarantine`/`reject`). Consider moving to a `mail.care-log.org` subdomain and switching the From address to a reply-friendly mailbox (`hello@`, `support@`). **Risk:** HIGH on the business outcome (every auth/transactional email is currently failing to end-users), LOW on the technical change (DNS record + Resend sender config). **Size:** XS (~30 min DNS + Resend) plus 24-48h DMARC propagation window. |
