...
|---|---|---|
| 🟢 Ready | 10 | TD-133 · TD-141 · TD-144 · TD-173 · UX-077 · UX-103b/105b · SEC-008 · SEC-009 · SEO-005 · PP-009 · ON-70 · ON-71 · ON-74 (+ rows in §3–§5 not enumerated) |
| 🔎 In review | 0 | — |
...
| TD-144 | 🟢 Ready · **P2 / Medium** | **Atomic OCR confirm via SECURITY DEFINER RPC (collapse audit + medication insert + status update into one tx)** | Surfaced 2026-05-15 by SEC-007 pre-plan opus-on-opus cycle 1 (cleared as out-of-scope follow-up). The OCR confirm route now does 3 sequential writes — audit insert, medication insert, optimistic-lock status update — via separate `supabaseAdmin.from()` calls. The optimistic-lock pattern (`UPDATE ... WHERE status = <prior>`) returns count=0 if a concurrent caller already flipped the row, but by that point the medication INSERT has already committed, leaving a duplicate medication + an audit row for a losing attempt. SEC-007 deliberately keeps per-attempt audit semantics (records 409-losers for forensics — good); but the underlying medication-duplication race needs an actual SQL transaction. Fix: SECURITY DEFINER PL/pgSQL function `confirm_ocr_job(job_id, org_id, drug_name, dosage, instructions)` that does `SELECT ... FOR UPDATE` on `ocr_jobs`, validates state, inserts medication, inserts audit, updates status — all in one tx. Route handler becomes a thin RPC caller with the same Zod validation + RLS check. **Risk:** MEDIUM (changes write semantics of a hot path; need pgTAP coverage of the race scenario via concurrent SELECT FOR UPDATE assertions). **Size:** M (~3–4h with tests). |
...
| TD-173 | 🟢 Ready · **P3 / Low** | — | — | **BriefingHandoffPanel test gap: no assertion on tab semantics (`role="tab"`)** | Surfaced 2026-05-17 by `/oop --from-sprint` on UX-065 (PR #585). The 6 tests in `apps/web/components/shifts/__tests__/BriefingHandoffPanel.test.tsx` cover render/loading/error/a11y heading hierarchy but the plan's acceptance #3 ("tab role correct per .claude/rules/ui-standards.md") has no test. **Fix:** add a vitest case asserting the Briefing tab button in `ShiftsPanel` has `role="tab"` and is reachable via Tab key. **Risk:** LOW. **Size:** XS (~15 min — 1 test). Also addresses /oop should-fix items (UX-058 → UX-065 JSDoc drift in BriefingHandoff.tsx, UTC-timestamp fragility note in summarizeSleep) — can bundle. |
