...
Last consolidated: **2026-04-16** (codebase scan same day). Last `/backlog-sync`: **2026-05-14 PM (SEC-001 audit + UX-045 dedup)** — `/sprint sec-001-ux-045` discovered UX-045 was a duplicate §1 row (PR #396 from Wave 13-A already shipped the fix; row was recreated by a later /impeccable critique that didn't notice). Removed the duplicate. SEC-001 read-only runbook audit added at `docs/project-info/runbooks/SECRETS_ROTATION-audit-2026-05-14.md`; rotation itself is human dashboard work and stays In progress. Remaining Ready = 30.
...
|---|---|---|
| 🟢 Ready | 34 | TD-111 · TD-120..123 · TD-129/130 · TD-133..136 · UX-041..044 · UX-048..050 · UX-053 · UX-065 · UX-077 · UX-103b/104b/105b · SEC-007 · SEC-008 · SEC-009 · SEO-005 · PP-009 · ON-70 · ON-71 · ON-74 (+ rows in §3–§5 not enumerated) |
| 🔎 In review | 0 | — |
...
| SEC-009 | 🟢 Ready · **P2 / Medium** · sequenced after SEC-001 | **Populate Vercel's Development environment tier with local-tier values** | Mitigates the contributing-factor that caused SEC-001 (default `vercel env pull` pulls Production). After rotation completes: in Vercel dashboard → Project → Settings → Environment Variables, for each variable add a "Development" override with a local-tier value: `SUPABASE_URL`/`SUPABASE_ANON_KEY`/`SUPABASE_SERVICE_ROLE_KEY` → the values from `supabase status -o env` (local Supabase JWTs); `STRIPE_WEBHOOK_SECRET` → Stripe test-mode webhook secret; `RESEND_API_KEY` → Resend test-mode key; `INNGEST_*` → Inngest dev/branch keys; `UPSTASH_REDIS_REST_TOKEN` → either a dev Upstash instance or omit (local rate-limit can no-op). Going forward, `vercel env pull --environment=development .env.local` (the `vepull` alias in `SECRETS_ROTATION.md` §3) grabs SAFE values. Closes the leak-vector permanently. **Risk:** LOW (additive — does not change Production or Preview tiers). **Size:** S (~30 min UI work, no code). Sequenced AFTER SEC-001 actually rotates so you're not splitting attention. |
| TD-134 | 🟢 Ready · **P3 / Low** | **Wire `supabase db diff --linked` into a pre-migration check script** | Pre-staging mitigation surfaced 2026-05-14 PM. Currently `supabase db diff --linked` is available CLI-side but not wired into anything. Add (1) a `pnpm migration-check` script (in repo root `package.json`) that runs `supabase db diff --linked --schema public` and prints any drift between local and linked-prod schema; (2) a one-line note in `.claude/CLAUDE.md` Development Workflow section saying to run it before opening any migration PR. Operator-discipline mitigation. **Explicitly out of scope here:** a CI gate. Rationale: `db diff --linked` requires prod credentials in CI (contradicts the SEC-001 incident's "don't credential more surfaces than needed" lesson) AND only catches the least-scary failure mode (schema drift) — not the actually-scary mode (concurrent-load locking, NOT NULL on big tables). For CI migration safety, see TD-136 (lint migration SQL for risky DDL patterns) which is a strictly better gate shape. **Risk:** LOW. **Size:** S (~30 min). |
| TD-135 | 🟢 Ready · **P3 / Low** | **PostHog feature-flag SDK wiring + 1 reference flag** | Pre-staging mitigation surfaced 2026-05-14 PM. PostHog is already in the stack for analytics (`apps/web/lib/posthog*`), and PostHog supports feature flags via the same SDK with zero added infra cost. Today there are 0 `isFeatureEnabled` call sites — "ship dark to 1 family, then 5, then everyone" is not possible without code-level conditionals. Add (1) server-side helper `getFeatureFlag(flag: string, distinctId: string): Promise<boolean>` in `apps/web/lib/featureFlags.ts`; (2) client-side React hook `useFeatureFlag(flag)` that reads the flag from PostHog's bootstrapped state; (3) one reference call site behind a `feature_test_flag` flag so future contributors have a pattern to copy; (4) ADR or short doc on the rollout pattern (1 user → 5 → 50 → everyone). PHI invariant continues to apply — `distinctId` is the anonymous UUID, never email. **Risk:** LOW (additive — no existing behavior changes). **Size:** S–M (2-3h including the ADR). |
| TD-136 | 🟢 Ready · **P3 / Low** | **CI gate: lint migration SQL for risky DDL patterns** | Better-shaped sibling of TD-134's dropped CI proposal. Run a static lint over any `supabase/migrations/*.sql` file added/modified in the PR diff, flagging risky DDL patterns that pass `db diff` but bite under concurrent load: `ALTER TABLE … ADD COLUMN … NOT NULL` without `DEFAULT` (rewrites every row), `CREATE INDEX` without `CONCURRENTLY` (blocks writes), `DROP COLUMN` (breaks deployed code reading it). Implementation options: (a) `sqlfluff` with ansi dialect + a small custom rules file; (b) lighter grep-based CI step that requires a `-- safe-migration: <reason>` SQL comment to pass when a risky pattern is detected. Reason-in-SQL comment is preferred over PR-description compliance because the justification lives WITH the migration file and survives rebase / squash. Pairs with the existing `~/.claude/skills/migration-safety/SKILL.md` (author-time judgment) — this CI gate catches the static patterns; the skill handles the judgment calls. **No prod credentials required** in CI (unlike TD-134's rejected CI variant). **Risk:** LOW. **Size:** S–M (2-3h for option (b); ~4h for sqlfluff). |
