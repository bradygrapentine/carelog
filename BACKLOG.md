...
|---|---|---|
| 🟢 Ready | 27 | TD-111 · TD-120..123 · **TD-128** · UX-041..044 · UX-048..050 · UX-053 · UX-065 · UX-077 · UX-103b/104b/105b · SEO-005 · PP-009 · ON-70 · ON-71 · ON-74 (+ rows in §3–§5 not enumerated) |
| 🔎 In review | 0 | — |
...
| TD-128 | 🟢 Ready | **Bump `next` 16.2.3 → 16.2.5 + refresh `protobufjs` / `@opentelemetry/*` pnpm overrides** | Discovered 2026-05-14 while preflighting `/sprint OOP` — `pnpm audit` job (made blocking by TD-29 / PR #165) failed on PR #464 with **14 high-severity vulns**, blocking every PR. Diagnosis from CI run 75949406283: (a) `next` 16.2.3 has 7 active high advisories (DoS via Server Actions, connection-handling, redirect, etc.), all patched in `>=16.2.5`; (b) the root `pnpm.overrides.protobufjs` pin to `8.0.1` set by TD-21 is now itself the vulnerable version (GHSA-685m-2w69-288q — DoS via unbounded recursion), patched in `>=8.0.2` — bump the override; (c) three transitive OpenTelemetry sub-packages need new pnpm overrides — `@opentelemetry/auto-instrumentations-node >=0.75.0`, `@opentelemetry/sdk-node >=0.217.0`, `@opentelemetry/exporter-prometheus >=0.217.0` (current `0.214.0` reaches via Sentry + Inngest + gRPC). Watch for TD-29 lockfile-split caveat: `apps/web/pnpm-lock.yaml` is separate from root; mobile is npm-format and pnpm overrides don't apply there (verify none of the 14 reach mobile before adding overrides). AC: `pnpm audit --prod --audit-level=high` exits 0 at root and in `apps/web/`. ~1–2 hr. **Blocks** PR #464 (OOP-* seed) and every subsequent PR. |
