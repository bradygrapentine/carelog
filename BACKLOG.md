...
|---|---|---|
| 🟢 Ready | 13 | TD-111 · TD-133 · TD-141 · TD-144 · TD-172 · UX-065 · UX-077 · UX-103b/105b · SEC-008 · SEC-009 · SEO-005 · PP-009 · ON-70 · ON-71 · ON-74 (+ rows in §3–§5 not enumerated) |
| 🔎 In review | 0 | — |
...
| TD-172 | 🟢 Ready · **P3 / Low** | — | — | **`memberships.invite` Sentry tag uses `path: "membership.error"` vs `caller.error` everywhere else** | Surfaced 2026-05-17 by `/oop --from-sprint` on TD-171 (PR #578). Tag inconsistency means alert dashboards filtering on `tags.path == "caller.error"` across the memberships router would miss invite-mutation captures. **Fix:** rename `path: "membership.error"` to `path: "caller.error"` at `apps/web/server/routers/memberships.ts:44` AND update the matching tag assertion in `apps/web/server/routers/__tests__/membershipsRouter.security.test.ts` (TD-171 transport-error test). **Risk:** LOW (tag-shape rename, no behavior change). **Size:** XS (~5 min — 2 strings + 1 test assertion). |
