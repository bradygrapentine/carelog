...
|---|---|---|
| 🟢 Ready | 16 | TD-133 · TD-141 · TD-174 · TD-175 · TD-177 · TD-178 · TD-179 · TD-180 · TD-181 · TD-182 · TD-183 · TD-184 · UX-077 · SEC-008 · SEC-009 · PP-009 · ON-70 · ON-71c · ON-71d · ON-74 (+ rows in §3–§5 not enumerated) |
| 🔎 In review | 1 | TD-185 (PR #613) |
...
| TD-185 | 🔎 In review · **P2 / Medium** · PR #613 | — | — | **Harden TD-170d drift script — shell injection + scope + allowlist** | Surfaced 2026-05-18 by `/oop --from-sprint` on TD-170d (PR #611). **Must-fix:** `git()` and `grepE2eForString()` interpolated filenames + removed strings into shell command strings (`execSync(\`git ${args}\`)`). A renamed file with `"`, `$()`, `;`, or backtick in the path becomes command injection. **Should-fix:** (a) `buildAddedStringSet` scoped whole-repo — lockfile/types addition could mask a real removal; (b) allowlist accepted opt-outs in any file (.md/.yml stealth); (c) `grepE2eForString` silently returned `[]` on missing `e2e/` via `grep || true`. **Fix in PR #613:** switched `git()` to `execFileSync("git", argv)`; replaced shell grep with in-process fs+regex scan (1 read per e2e file vs N forks per string); scoped ADDED-set to UI files only; restricted allowlist to code files; early-exit if `e2e/` missing. All 14 unit tests + 2259 vitest tests pass. Retro-validation re-confirmed against PRs #525/#528/#529 (exit 1, correct drift counts). |
...
| TD-180 | 🟢 Ready · **P3 / Low** | — | — | **Extract `useEditMode<TForm>` hook to DRY edit-mode + mutation pattern across UX-103b / UX-105b** | Surfaced 2026-05-17 by `/oop --from-sprint`. Both `CareTeamList.tsx` and `EmergencyFooterCard.tsx` reimplement: open/close toggle, mutation hook with `onSuccess`/`onError` callbacks, generic submitError state, `router.refresh()` on success, cancel-resets-form. Proposed hook signature: `useEditMode<TForm>({ initial, mutation, onSuccessRefresh: true })` returns `{ isEditing, open, cancel, submit, error, isPending }`. Unlocks UX-066/UX-103/UX-105 follow-ups + future inline-edit panels. **Risk:** LOW (additive). **Size:** S (~1.5h — write hook + retrofit both components + tests). |
| TD-181 | 🟢 Ready · **P2 / Medium** | — | — | **Extend `carelog/no-phi-in-analytics` ESLint rule to Sentry options + Resend args; retire grep-based PHI sentinel test** | Surfaced 2026-05-17 by `/oop --from-sprint` on ON-71 Phase 2 (PR #599). The current rule only catches PHI in `posthog.identify/capture` and `Sentry.setUser/setContext`. New surfaces left uncovered: `Sentry.captureException(_, { tags|extra|contexts: ... })` (caught today via a brittle file-grep test in `refillAlert.test.ts`), `resend.emails.send({ subject|html: ... })` if PHI flows there. The file-grep sentinel test catches `refillAlert.ts` only; a regression in `burnoutAlert.ts`/`weeklyDigest.ts`/`messagingPush.ts` would land silently. **Fix:** extend the ESLint rule to (a) flag PHI field name literals + `input`/`patch`/`event` spreads inside `Sentry.captureException` options, (b) flag PHI field names in `resend.emails.send` `subject`/`html`/`from`/`headers` keys. After landing, remove the grep test from `refillAlert.test.ts` PHI sentinel block. **Risk:** MEDIUM (rule changes can be noisy — review false-positive rate before turning errors on). **Size:** M (~3h — rule implementation + sweep existing call sites + retire grep test). |
