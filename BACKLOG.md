...
Last consolidated: **2026-04-16** (codebase scan same day). Last `/backlog-sync`: **2026-05-10 PM-3 (post recipient-profile wave)** — UX-103/104/105 shipped today via PRs #451/#452/#453 (recipient profile data wiring); plus 7 stories from this morning's smoke-walk + TD-118/119 wave (#438–#448). 4 follow-up TD/UX rows added from the Sonnet adversarial gate (TD-120/121/122 + UX-103b/104b/105b). Remaining Ready = 25.
...
|---|---|---|
| 🟢 Ready | 30 | TD-111 · TD-120..127 · UX-041..044 · UX-048..050 · UX-053 · UX-065 · UX-077 · UX-103b/104b/105b · SEO-005 · PP-009 · ON-70 · ON-71 · ON-74 (+ rows in §3–§5 not enumerated) |
| 🔎 In review | 0 | — |
...
| TD-124 | 🟢 Ready · **P2** | **Extend `supabase/seed.sql` to create a working coordinator + recipient + memberships row** | Surfaced 2026-05-10 PM smoke walk. The seed creates `auth.users` for `brady.grapentine@gmail.com` but the comment promises "as a coordinator with a professional-plan org" — and the `INSERT INTO public.organizations / care_recipients / memberships` statements that follow are silently failing (verified: `select count(*) from organizations` returns 0 after `supabase db reset`). Result: every fresh local dev environment can't test any post-onboarding surface (recipient profile, journal, settings with content, dashboard with care teams) without manually clicking through onboarding first. Today's UX-103/104/105 wave shipped to a recipient profile route that wasn't testable in the live-test smoke walk for this reason. Fix: debug the failing inserts (likely an RLS policy or a missing column reference); ensure `supabase db reset` produces a coordinator + 1 recipient + 1 membership ready for sign-in. ~30 min. |
| TD-125 | 🟢 Ready · **P3** | **Silence Sentry 429s in local dev** | Surfaced 2026-05-10 PM smoke walk. Every page load on local dev hits `POST http://localhost:3000/monitoring` and gets back `429 Too Many Requests` — Sentry self-monitoring rate-limits on the free-tier quota with normal browsing. Pollutes the browser console + adds a failed network request to every page (which can mask real errors during debugging). Fix: gate Sentry SDK init on `process.env.NODE_ENV === 'production'` OR `SENTRY_ENABLED === 'true'`, OR drop `tracesSampleRate` to 0 in dev. Likely touches `apps/web/instrumentation.ts` + `instrumentation-client.ts`. ~15 min. |
| TD-126 | 🟢 Ready · **P3** | **`priority` prop on the marketing landing hero image (LCP fix)** | Surfaced 2026-05-10 PM smoke walk. Next.js logs a console warning on `/`: `Image with src "/images/hero-4.png" was detected as the Largest Contentful Paint (LCP). Please add the loading="eager" property if this image is above the fold.` Fix: add `priority` prop to the Next `<Image>` rendering the landing hero. Touches one component (likely `apps/web/components/marketing/HeroSection.tsx` or wherever `/images/hero-4.png` is referenced). Pairs with SEO-005 (Core Web Vitals tightening) — could be folded in. ~5 min standalone. |
| TD-127 | 🟢 Ready · **P2** | **Document `needs-phi-review` label removal protocol — or auto-validate** | Surfaced 2026-05-10 PM. Two PRs last evening had `needs-phi-review` auto-attached and stalled until manual signoff: #453 (touched identity_vault), and the label-attach behavior is what BLOCKED auto-merge — Vercel's failure was informational only. There's no documented protocol for who removes the label, what review must precede removal, or whether the orchestrating Claude session is allowed to self-clear. Today's fix was: orchestrator manually inspected the diff against ADR-0001, posted a review comment, removed the label. That worked but isn't reproducible without the protocol. Two options: (a) **document the protocol** in ADR-0001 (or a new ADR) defining the bar — diff inspected against ADR-0001 PHI rule, posted review comment, label removable by author; covers both human + agent cases; (b) **automate** via a CI step that AST-walks the diff for `posthog.identify`/`Sentry.setUser`/`identity_vault` reads + asserts shape against an explicit allowlist, removes label on green. (a) is ~1 hr; (b) is ~3-4 hr but eliminates the manual step. Recommend (a) first to unstick PRs, file (b) as a follow-up. |
