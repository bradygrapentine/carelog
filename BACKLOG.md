...
| TD-148 | 🟢 Ready · **P3 / Low** | — | — | **Bump Inngest TS SDK above 3.54.0 threshold** | Inngest dashboard surfaced "v3 TS SDK Security Alert: upgrade to v3.54.0 or later" during SEC-001 verify on 2026-05-16. Currently pinned at exactly `3.54.0` in `apps/web/package.json` (caret) + pnpm lockfile. Nominally compliant (at the threshold) but flagged. Bump to latest 3.x stable, run vitest, redeploy. **Risk:** LOW. **Size:** XS (~15 min). |
| TD-152 | 🟢 Ready · **P2 / Medium** | — | — | **Wire Sentry around Quick log handlers + `care_events` REST failure path** | Surfaced 2026-05-16 by post-SEC-001 happy-path live-test. TD-149 and TD-150 both fail silently in prod — zero Sentry breadcrumbs, zero captureException calls — which is why the bugs went undetected before tonight's live test. Audit the failing surfaces and add appropriate `Sentry.captureException` / `Sentry.addBreadcrumb` instrumentation: (a) wrap Quick log click handlers in try/catch with breadcrumb on every item-click; (b) attach Sentry.setContext on the `care_events` REST client so 5xx responses are captured with the org_id, recipient_id, and query shape. **Observability target:** any future silent-failure on these two paths should appear in Sentry within 30s. **Risk:** LOW (instrumentation only). **Size:** S (~1-2h). |
...
| TD-162 | 🟢 Ready · **P3 / Low** | — | — | **Apply TD-149 `count: "estimated"` fix to RLS-protected `count: "exact"` calls in `ai.ts`** | Surfaced 2026-05-17 by `/oop --from-sprint` review of the live-test-p1s sprint. `apps/web/server/routers/ai.ts:113,120,129` has three `count: "exact"` calls via the RLS-scoped `ctx.supabase` client against `care_events` and `shifts` — same pattern that caused the TD-149 503 on `DashboardClient.tsx`. Background AI context-builder reads (latency-bounded, not user-blocking), but identical RLS-eval cost characteristics. Apply the same fix: swap to `count: "estimated"` per the rationale in `docs/research/2026-05-17-td-149-care-events-head-503.md`. **Out of scope:** all `supabaseAdmin` `count: "exact"` calls (RLS-bypassing, safe). **Risk:** LOW. **Size:** XS (~15 min — three single-line edits + targeted vitest). |
