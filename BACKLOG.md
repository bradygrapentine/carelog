...
|---|---|---|
| 🟢 Ready | 18 | TD-133 · TD-141 · TD-170d · TD-170e · TD-174 · TD-175 · TD-177 · TD-178 · TD-179 · TD-180 · TD-181 · TD-182 · TD-183 · TD-184 · UX-077 · SEC-008 · SEC-009 · PP-009 · ON-70 · ON-71c · ON-71d · ON-74 (+ rows in §3–§5 not enumerated) |
| 🔎 In review | 0 | — |
...
| TD-170d | 🟢 Ready · **P2 / Medium** | — | — | **E2E string-drift lint rule (prevention for UX-04x class of regression)** | Surfaced 2026-05-18 by the TD-170b/c arc. Three copy-audit PRs landed 2026-05-15 (UX-048/049/050), each rename'd UI strings without sweeping E2E. Cost: 3 days of chronic-red E2E + 4 firefighting PRs to resolve. **Fix shape:** pre-commit or PR-time check — when a PR's diff renames a JSX text node OR a string literal in `apps/web/**/*.tsx`, grep `e2e/**/*.ts(x)` for the old string. Any match → fail with "rename also touches E2E: update or add `// e2e:no-references` opt-out". **Risk:** LOW (test-only enforcement; allowlist for false-positives). **Size:** S (~2-3h — hook script + CI integration + 1-2 false-positive carve-outs). |
| TD-170e | 🟢 Ready · **P3 / Low** | — | — | **`e2e/medication-tagging.spec.ts:44` strict-mode violation — two `Add medication` buttons** | Surfaced 2026-05-18 post-#608 by E2E [2/3]. `getByRole('button', { name: /add medication/i })` matches 2 elements: canonical `<button>Add medication</button>` AND `<button data-testid="add-medication-btn">+ Add medication</button>`. Test was previously failing earlier in the flow (on "Share update") so this assertion was never reached. **Fix:** specify the test to use `getByTestId('add-medication-btn')` OR `getByRole('button', { name: 'Add medication', exact: true })` per the Playwright suggestion in the error. **Risk:** LOW. **Size:** XS (~10 min). Note: this is a pre-existing latent bug, not a TD-170c regression — the dual-button UI predates the 2026-05-15 redesign. |
