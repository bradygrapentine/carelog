...
| 🔴 Blocked | 0 | — |
| 🧊 Deferred | 15 | §5 ON-55 (ON-69 struck dup) · §1 UX-066 (superseded) · UX-035 (BriefHero now wired) · UX-051 (caresync.app DNS not provisioned) · TD-126 (LCP premise wrong; 132ms local) · SEO-003 (no /carezone-alternative; /about lacks step content) · ON-72/73 (Phase 3/4 prereqs) · §6 UX-08/09/11/22/23/24 · §3 PP-013 |
...
| OOP-010 | 🟢 Ready | **Mobile `AsyncStorage` key registry** | Magic strings (`"onboarding_complete"` at `apps/mobile/lib/onboarding.ts:3` + 2 others) with no namespace/versioning. New: `apps/mobile/lib/asyncStorageKeys.ts` exporting `KEYS = { onboarding, pushToken, inviteToken } = "v1:carelog:…"`. Migration path: try new key first, fall back to old on first read, write back under new. **Risk:** LOW. **Size:** S (1h). Plan doc §Wave C. |
