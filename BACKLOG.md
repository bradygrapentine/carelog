...
|---|---|---|
| 🟢 Ready | 22 | TD-111 · TD-133 · TD-141 · TD-142 · TD-143 · TD-144 · TD-145 · UX-053 · UX-065 · UX-077 · UX-103b/104b/105b · SEC-008 · SEC-009 · SEO-005 · PP-009 · ON-70 · ON-71 · ON-74 (+ rows in §3–§5 not enumerated) |
| 🔎 In review | 0 | — |
...
| TD-145 | 🟢 Ready · **P3 / Low** | **Annotate `ocr_audit_log` index creation with `safe-migration` comment (DDL lint CI fix-forward)** | SEC-007 migration `20260516010000_create_ocr_audit_log.sql` ships two `CREATE INDEX` statements without `CONCURRENTLY` — failed the `Lint risky DDL patterns` CI check post-merge. Indexes were built on a brand-new empty table (zero rows blocked → zero rows of impact), but the script doesn't know the parent table is new. Fix: tiny migration `20260516020000_annotate_ocr_audit_indexes.sql` that's a no-op SELECT with a `-- safe-migration: ocr_audit_log table created same migration; indexes built on empty table` comment. OR: update `scripts/migration-lint.sh` to whitelist `CREATE INDEX` immediately following `CREATE TABLE` in the same file. Lint script change is the right fix (prevents future false positives) — but is itself harness work. **Risk:** LOW (CI advisory only; no prod impact). **Size:** XS (~15min). |
