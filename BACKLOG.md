...
Last consolidated: **2026-04-16** (codebase scan same day). Last `/backlog-sync`: **2026-05-15 (adversarial-gate-cleanup sprint close)** — `/sprint adversarial-gate-cleanup` shipped TD-130 + TD-122 via single direct-implementation wave (PRs #510/#511). Real Opus ran `/opus-on-opus` this time (recovered from earlier degradation) and cleared the plan at 0 must-fix. Two Opus-surfaced Should-fix items seeded as follow-ups: TD-139 (PDF export `note`/`notes` parallel rendering) + TD-140 (memberships test filter-contract assertions). §0 net Ready count unchanged at 30 (-2 shipped + 2 new). Remaining Ready = 30.
...
|---|---|---|
| 🟢 Ready | 30 | TD-111 · TD-120/121/123 · TD-129 · TD-133 · TD-137..140 · UX-044 · UX-048..050 · UX-053 · UX-065 · UX-077 · UX-103b/104b/105b · SEC-007 · SEC-008 · SEC-009 · SEO-005 · PP-009 · ON-70 · ON-71 · ON-74 (+ rows in §3–§5 not enumerated) |
| 🔎 In review | 0 | — |
...
| TD-139 | 🟢 Ready · **P3 / Low** | **PDF export: render `payload.note` and `payload.notes` alongside `payload.text`** | Surfaced 2026-05-15 by Opus pre-merge review of TD-130 (PR #510). `JournalPayload` (`apps/web/lib/careEvent.ts:49-54`) defines `text`, `note`, `notes` as parallel free-text slots. `VisitSummary.tsx:387-390` already reads `note`/`notes`, but `apps/web/app/api/history/export/pdf/route.tsx:231-235` only renders `payload.text`. Coordinators whose journal entries write to `note`/`notes` (which is many entries — `text` is only observed in `handoffSummary.ts:146` and the PDF route per `careEvent.ts:46`) get silent blanks in their PDF export. Add the parallel-slot rendering with the same `typeof === "string"` guard pattern from TD-130. Verify `lib/buildHistoryExport.ts` snapshot already includes these fields (probably yes, since `payload` is `Record<string, unknown>`). **Risk:** LOW (additive — non-string slots still skipped). **Size:** XS (~30min). |
| TD-140 | 🟢 Ready · **P3 / Low** | **memberships test: assert filter contracts on `getCareTeamForRecipient`** | Surfaced 2026-05-15 by Opus pre-merge review of TD-122 (PR #511). The new thenable mock from TD-122 wraps all filters (`select`, `eq`, `or`, `not`) in `vi.fn(() => chain)`, enabling direct call-arg assertions. Today the `getCareTeamForRecipient` tests at `apps/web/server/repositories/__tests__/membershipsRepository.test.ts:173+` only await the function's final result — nothing asserts specific filter shapes, so a regression that drops a `.not()` filter on the prod query would still pass silently. Add 1-3 `expect(chain.eq).toHaveBeenCalledWith("org_id", ORG_ID)` style assertions to one or two of the existing happy-path tests to lock the prod query shape. **Risk:** LOW (test-only). **Size:** XS (~30min). |
