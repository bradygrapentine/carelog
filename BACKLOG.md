...
|---|---|---|
| 🟢 Ready | 13 | TD-111 · TD-133 · TD-141 · TD-144 · TD-168 · UX-065 · UX-077 · UX-103b/105b · SEC-008 · SEC-009 · SEO-005 · PP-009 · ON-70 · ON-71 · ON-74 (+ rows in §3–§5 not enumerated) |
| 🔎 In review | 0 | — |
...
| TD-155 | 🟢 Ready · **P3 / Low** | — | — | **Confirm OTP rate limiter is keyed on email, not IP** | Surfaced 2026-05-17 by Cowork auth-unhappy suite run `1778990299` scenario 3. Six rapid OTP requests within ~200ms returned `200, 429, 429, 429, 429, 429` — limiter fires from attempt 2 not 5-6 as runbook expected. No 500s (security intact), but if the limiter is **IP-keyed** rather than **email-keyed**, a single household IP requesting OTP for multiple family members (the actual carelog use case) would get blocked. Verify in the Upstash rate-limit middleware (likely `apps/web/lib/rate-limit.ts` or similar) that the key combines `email` not just `ip`. If IP-keyed, refactor to email-keyed with sensible burst window. **Risk:** LOW. **Size:** S (~1h — read code, possibly refactor + test). |
...
| TD-168 | 🟢 Ready · **P3 / Low-Med** | — | — | **`changeRole` + `remove` tRPC mutations echo raw Postgres error messages to client** | Surfaced 2026-05-17 by `/oop --from-sprint` on TD-167 (PR #569). Same class of leak as TD-167, three more sites in `apps/web/server/routers/memberships.ts`: (1) `changeRole` line ~184 passes `updateError.message` as TRPCError message; (2) `remove` ~248 passes `countError.message`; (3) `remove` ~272 passes `deleteError.message`. **Fix:** mirror the TD-167 pattern — generic `"Failed to change role"` / `"Failed to remove member"` client message + `Sentry.captureException(error, { tags: { component: "memberships.changeRole" \| "memberships.remove", path: "update.error" \| "count.error" \| "delete.error" } })`. **Risk:** LOW. **Size:** XS (~20 min — 3 error branches + 3 vitest cases in `membershipsRouter.security.test.ts`). |
