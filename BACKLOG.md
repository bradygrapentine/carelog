...
|---|---|---|
| 🟢 Ready | 13 | TD-111 · TD-133 · TD-141 · TD-144 · TD-171 · UX-065 · UX-077 · UX-103b/105b · SEC-008 · SEC-009 · SEO-005 · PP-009 · ON-70 · ON-71 · ON-74 (+ rows in §3–§5 not enumerated) |
| 🔎 In review | 0 | — |
...
| TD-171 | 🟢 Ready · **P3 / Low** | — | — | **`memberships.invite` has the same membershipError-on-transport gap that TD-169 fixed on changeRole/remove** | Surfaced 2026-05-17 by `/oop --from-sprint` on TD-169 (PR #576). Structurally identical to TD-169: `apps/web/server/routers/memberships.ts:41-48` throws bare `FORBIDDEN` when `membershipError` is set, with no Sentry capture distinguishing real DB transport failure from a legitimate "no membership" PGRST116. **Fix:** insert `if (membershipError && membershipError.code !== "PGRST116") { Sentry.captureException(wrapAdminError(membershipError), { tags: { component: "memberships.invite", path: "membership.error" } }); }` before the existing FORBIDDEN throw. **Risk:** LOW. **Size:** XS (~10 min — 1 branch + 1 vitest case). |
