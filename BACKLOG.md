...
|---|---|---|
| 🟢 Ready | 12 | TD-111 · TD-133 · TD-141 · TD-144 · UX-065 · UX-077 · UX-103b/105b · SEC-008 · SEC-009 · SEO-005 · PP-009 · ON-70 · ON-71 · ON-74 (+ rows in §3–§5 not enumerated) |
| 🔎 In review | 0 | — |
...
|---|---|---|---|---|---|
| TD-147 | 🟢 Ready · **P3 / Low** | — | — | **Consolidate decision-tracking surfaces (adr/ vs UX_DECISIONS.md vs plans/audit "Guiding decisions")** | Surfaced 2026-05-16 by `/insights-from-rag`. Three sources of truth for "what was decided": `docs/adr/README.md` (architecture), `docs/project-info/product/UX_DECISIONS.md` (UX/language), and the "Guiding decisions (decide once, applies everywhere)" section in `docs/plans/audit-remediation-2026-05-09.md`. Future contributors don't know which surface is canonical. **How (fresh session):** 1) Read `CLAUDE.md` first to understand carelog's doc conventions and which surface is currently authoritative. 2) Run `/insights-from-rag carelog` to confirm the finding is still current (vault may have moved). 3) Audit all three surfaces. 4) Propose a single canonical location (likely `docs/adr/` since `/write-adr` points there) and a migration plan for the UX decisions + plan-embedded guiding decisions — OR document UX_DECISIONS as the canonical product-decision surface alongside adr/ for architecture (two by intent, not by accident). 5) Update CLAUDE.md to name the convention. **Done when:** one canonical decision-doc surface (or two explicitly differentiated) + CLAUDE.md note + plan-embedded "Guiding decisions" migrated out. Sister stories: weather-forecaster (shipped), fathom DOC-1 (backfill, not consolidation), auto-rebaser DOC-1 (parked, v2 released). |
...
| TD-153 | 🟢 Ready · **P2 / Medium** | — | — | **Reconcile happy-path runbook with current prod flow (or rebuild missing screens)** | Surfaced 2026-05-16 by post-SEC-001 happy-path live-test (Cowork run `1778988534`). The runbook at `docs/runbooks/post-sec-001-happy-path.md` §2 describes screens that no longer exist in production: (1) "Choose a plan" Stripe Checkout step — gone, no $14/mo plan-selection flow; (2) `/journal` route — italicized text on dashboard, not a link; (3) `/medications` route — "Add one in Medications →" link 404s; (4) Team / invite UI in Settings — Settings has Profile/Notifications/Language/Refer/Danger zone only, no team-invite affordance; (5) auth is passwordless 6-digit OTP, not magic-link. **Decision needed:** are these missing surfaces intentional (e.g. Stripe Checkout deferred, journal/medications consolidated into Quick log + dashboard) or unintended regressions that need rebuilding? **Path A (rewrite runbook):** update §2 to match current prod (OTP, dashboard-only flow, no plan selection). **Path B (rebuild screens):** restore the missing surfaces if they're intended product. Likely a mix. **Blocks:** future live-tests against the current runbook produce noisy SKIPs. **Risk:** LOW (doc work) or M (product rebuild — depends on path). **Size:** S (Path A) or M+ (Path B). |
