...
|---|---|---|
| 🟢 Ready | 33 | TD-111 · TD-120..123 · TD-129/130 · TD-133..135 · UX-041..044 · UX-048..050 · UX-053 · UX-065 · UX-077 · UX-103b/104b/105b · SEC-007 · SEC-008 · SEC-009 · SEO-005 · PP-009 · ON-70 · ON-71 · ON-74 (+ rows in §3–§5 not enumerated) |
| 🔎 In review | 0 | — |
...
| TD-134 | 🟢 Ready · **P3 / Low** | **Wire `supabase db diff --linked` into a pre-migration script** | Pre-staging mitigation surfaced 2026-05-14 PM. Currently `supabase db diff --linked` is available CLI-side but not wired into anything — pre-commit, CI, or runbook. Add (1) a `pnpm migration-check` script in `apps/web/package.json` (or repo root) that runs `supabase db diff --linked --schema public` and prints any drift between local and linked-prod schema; (2) a one-line note in `.claude/CLAUDE.md` "Development Workflow" section saying to run it before opening any migration PR; (3) optional CI step that fails a PR if migration files in the diff aren't accompanied by a `db diff` snapshot in the PR description. Operator-discipline mitigation — catches schema drift before merge. **Risk:** LOW. **Size:** S (~30 min for script + docs; +30 min if adding CI gate). |
