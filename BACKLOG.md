...
|---|---|---|
| 🟢 Ready | 12 | TD-133 · TD-141 · TD-174 · TD-175 · TD-176 · TD-177 · TD-178 · TD-179 · TD-180 · UX-077 · SEC-008 · SEC-009 · PP-009 · ON-70 · ON-71 · ON-74 (+ rows in §3–§5 not enumerated) |
| 🔎 In review | 0 | — |
...
| TD-176 | 🟢 Ready · **P3 / Low** | — | — | **Standardize Sentry `component` tag convention for API routes vs tRPC routers** | Surfaced 2026-05-17 by `/oop --from-sprint` on TD-144 (PR #589). `apps/web/server/routers/memberships.ts` uses `component: "memberships.<method>"` (router name dot procedure name); the new `apps/web/app/api/ocr/confirm/route.ts` uses `component: "ocr.confirm"` (resource dot verb). Not a functional bug but a tag-shape divergence that complicates Sentry dashboard filters. **Fix:** pick one convention (suggest `api.ocr.confirm` for API routes to distinguish from `<router>.<method>` for tRPC) and document in `docs/project-info/technology/CODE_STANDARDS.md`. **Risk:** LOW. **Size:** XS (~20 min — sweep existing API-route Sentry tags + add to standards doc). |
