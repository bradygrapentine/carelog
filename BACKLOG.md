...
|---|---|---|
| 🟢 Ready | 17 | TD-133 · TD-141 · TD-170b · TD-174 · TD-175 · TD-177 · TD-178 · TD-179 · TD-180 · TD-181 · TD-182 · TD-183 · TD-184 · UX-077 · SEC-008 · SEC-009 · PP-009 · ON-70 · ON-71c · ON-71d · ON-74 (+ rows in §3–§5 not enumerated) |
| 🔎 In review | 0 | — |
...
| TD-170b | 🟢 Ready · **P1 / High** | — | — | **Root-cause E2E chronic-red on main (TD-170 helper fix incomplete)** | Surfaced 2026-05-18 by `/sprint td-87` preflight. `gh run list --branch main --workflow=CI --limit 15` shows 0 success — uniform `cancelled`/`failure` across all E2E [1/3..3/3] jobs. PR #592 (TD-170) shipped a 2-line `getByRole({ name: /Verify code/ })` helper fix on 2026-05-17 expecting to restore green; CI history shows it did not. Hypotheses (carry forward from TD-170 row + new): (1) supabase JWT-mint env drift in `ci.yml:283–296`; (2) playwright wait selectors brittle on Next 16 streaming; (3) Vercel preview env race vs. the locally-served build. **Action:** read latest failed CI run log + diff against last known-green run (find via `gh run list --branch main --status success --limit 1`), produce a ranked hypothesis list with file:line evidence. **Risk:** HIGH — every PR auto-merges on required checks while E2E is bypassed; real regressions slip. **Size:** M (~2–4h investigation; fix unknown until root cause). |
