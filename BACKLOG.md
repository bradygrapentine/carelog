...
| UX-109 | 🟢 Ready | **Unify palette: green + burnt orange across marketing AND app — drop the purple/Sage split** | Owner ask 2026-05-09. Today marketing is locked to the original purple brand (`#7c3aed`, PR #269 light-only lock) and the app shell defaults to the Sage parlor theme (PR #351). Owner wants ONE palette everywhere: primary green, secondary burnt orange. Scope: (1) replace `--color-primary` and `--color-secondary` (+ subtle variants) in `apps/web/app/globals.css`; (2) refresh marketing hero / `BriefHero` accents; (3) audit every raw `#7c3aed` / `#d97706` / Sage token reference; (4) regenerate any brand asset that bakes the old hex (favicon, OG images, signin block). Tertiary coral `#e76f51` stays unless owner says otherwise. Pairs with UX-110 (toggle removal) and UX-111 (post-login dark mode) — keep them sequenced. Concrete green/burnt-orange hex values TBD before any code changes. |
| UX-110 | 🟢 Ready | **Remove dark-mode toggle from marketing/landing toolbar AND from post-login app shell** | Owner ask 2026-05-09. Marketing was intentionally light-only locked in PR #269 but the toggle is still rendered, which is misleading. Post-login, dark mode is broken (see UX-111) and owner wants a single light scheme everywhere anyway. Scope: drop the `ThemeSwitcher` mount from the marketing nav AND from the app toolbar; keep the underlying `next-themes` provider in case we re-introduce themes later, but do not render any user-facing toggle. Pairs with UX-109 (palette unification). |
| UX-111 | 🟢 Ready | **Dark mode visibly broken on app shell post-login** | Repro 2026-05-09: signed in via local Inbucket, surfaces remain light (or partially themed) when toggling dark. Likely a missing `dark:` variant pass on the `(app)` shell after the Sage default flip (PR #351) or a stale token reference. **Note:** UX-110 will retire the toggle entirely, so this row only needs to ship if we keep dark mode as an option; otherwise close as won't-fix once UX-110 lands. Investigate before deciding whether to fix or retire. |

### Onboarding bugs (ON-75)

| ID | Status | Story | Notes |
|---|---|---|---|
| ON-75 | 🟢 Ready | **Onboarding wizard — care team setup gets stuck mid-flow** | Repro 2026-05-09: signed in fresh via local Inbucket and could not complete care team creation through the onboarding wizard. Symptom unclear (button no-op vs validation error vs silent failure) — needs full repro: which step, what was entered, console + network log. Touches `/onboarding` flow + `memberships` / `organizations` mutations. Block-level issue for first-run experience; treat as P1 once repro is captured. |
