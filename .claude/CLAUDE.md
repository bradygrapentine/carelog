...
- **Renaming a required CI job is a TWO-part change.** A workflow edit that renames or splits a job whose name is in branch protection's required-status-check list (`Lint`, `Typecheck`, `Web — web/node/a11y`, `RLS pgTAP tests`, etc.) strands its own PR: the old required context never reports → `mergeStateStatus: BLOCKED`, auto-merge waits forever even though all checks are green. The branch-protection update is admin-only and NOT capturable in the PR diff. When renaming/splitting a CI job, also `gh api -X PATCH repos/bradygrapentine/carelog/branches/main/protection/required_status_checks` to swap the context name. Past incident 2026-05-19: TD-198 renamed `Web — unit tests` → 3-way matrix `Web — web/node/a11y`; PR #649 stuck until the required-context list was swapped.
