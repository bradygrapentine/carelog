...
describe("journal payload", () => {
  ...
  it("rejects empty text", () => {
    expect(() => validatePayload("journal", { text: "" })).toThrow();
  });
  ...
  it("rejects missing text field", () => {
    expect(() => validatePayload("journal", { mood: "good" })).toThrow();
  });
});
...
describe("appointment payload", () => {
  it("accepts valid appointment with only required fields", () => {
    const result = validatePayload("appointment", {
...
describe("symptom payload", () => {
  ...
  it("rejects invalid appetite value", () => {
    expect(() =>
      validatePayload("symptom", { appetite: "excellent" }),
    ).toThrow();
  });
});
...
describe("unknown event type", () => {
  it("throws for unknown event type", () => {
    expect(() => validatePayload("unknown" as any, { text: "test" })).toThrow();
  });
});

// TD-210: named caps pinned to their literal values (hard-coded, not derived
// from the export) so an accidental change to the const is caught here.
describe("payload length caps (TD-210)", () => {
  it("JOURNAL_TEXT_MAX is 10000", () => {
    expect(JOURNAL_TEXT_MAX).toBe(10000);
  });

  it("HANDOFF_NOTE_MAX is 2000", () => {
    expect(HANDOFF_NOTE_MAX).toBe(2000);
  });

  it("journal text accepts exactly JOURNAL_TEXT_MAX chars and rejects one more", () => {
    expect(() =>
      validatePayload("journal", { text: "a".repeat(10000) }),
    ).not.toThrow();
    expect(() =>
      validatePayload("journal", { text: "a".repeat(10001) }),
    ).toThrow();
  });

  it("shift handoff_note accepts exactly HANDOFF_NOTE_MAX chars and rejects one more", () => {
    const base = {
      shift_id: "18dc6d19-6712-4b26-8797-b4e544e01b84",
      action: "completed",
    };
    expect(() =>
      validatePayload("shift", { ...base, handoff_note: "a".repeat(2000) }),
    ).not.toThrow();
    expect(() =>
      validatePayload("shift", { ...base, handoff_note: "a".repeat(2001) }),
    ).toThrow();
  });
});
