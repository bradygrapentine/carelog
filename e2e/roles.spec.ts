...
const COORDINATOR_EMAIL = 'e2e-journal@test.com'
...
async function setupRoleTest(browser: any, role: string, inviteeEmail: string) {
  ...
  const inviteUrl = await sendInviteAndGetUrl(coordinatorPage, inviteeEmail, role)

  const { page: inviteePage, ctx: inviteeCtx } = await acceptInviteAsNewUser(browser, inviteUrl, inviteeEmail)
...
test('supporter sees read-only message instead of entry form', async ({ browser }) => {
  const email = roleEmail('supporter')
  const { inviteePage, coordinatorCtx, inviteeCtx } = await setupRoleTest(browser, 'supporter', email)
  try {
    await expect(inviteePage.getByText("You're here as a Supporter")).toBeVisible()
    await expect(inviteePage.getByPlaceholder('Share how today went...')).not.toBeVisible()
  } finally {
...
test('supporter does not see flag button on journal entries', async ({ browser }) => {
  const email = roleEmail('supporter')
  const { coordinatorPage, coordinatorCtx, inviteePage, inviteeCtx } = await setupRoleTest(browser, 'supporter', email)
  try {
    const entryText = 'Supporter flag test ' + Date.now()
    const textarea = coordinatorPage.getByPlaceholder('Share how today went...')
    await textarea.fill(entryText)
    await coordinatorPage.waitForSelector('text=Share update', { timeout: 3000 })
    await coordinatorPage.click('text=Share update')
    await expect(textarea).toHaveValue('', { timeout: 12000 })

    await inviteePage.reload()
    await expect(inviteePage.getByText('Care team')).toBeVisible({ timeout: 15000 })
    const entryCard = inviteePage.locator('[data-testid="journal-entry"]', { hasText: entryText })
...
test('caregiver sees the entry form', async ({ browser }) => {
  ...
  const { inviteePage, coordinatorCtx, inviteeCtx } = await setupRoleTest(browser, 'caregiver', email)
  try {
    await expect(inviteePage.getByPlaceholder('Share how today went...')).toBeVisible()
    await expect(inviteePage.getByText("You're here as a Supporter")).not.toBeVisible()
  } finally {
    await coordinatorCtx.close()
    await inviteeCtx.close()
  }
...
test('caregiver can write a journal entry', async ({ browser }) => {
  ...
  try {
    ...
    await expect(textarea).toHaveValue('', { timeout: 12000 })
    await expect(inviteePage.getByText(entryText)).toBeVisible({ timeout: 5000 })
  } finally {
    await coordinatorCtx.close()
    await inviteeCtx.close()
  }
...
test('aide sees the entry form and can flag entries', async ({ browser }) => {
  ...
  const { inviteePage, coordinatorCtx, inviteeCtx } = await setupRoleTest(browser, 'aide', email)
  try {
    await expect(inviteePage.getByPlaceholder('Share how today went...')).toBeVisible()
    await expect(inviteePage.getByText("You're here as a Supporter")).not.toBeVisible()
