...
export async function navigateToJournal(page: Page): Promise<void> {
  await Promise.race([
    page.waitForSelector('button:has-text("View care journal")', { timeout: 15000 }),
    page.waitForSelector('a:has-text("Set up a care team")',     { timeout: 15000 }),
  ])

  const hasCareTeam = await page.locator('button:has-text("View care journal")').count() > 0
...
export async function sendInviteAndGetUrl(page: Page, email: string, role: string): Promise<string> {
  ...
  if (!match) throw new Error('No URL found in invite dialog: ' + dialog.message())
  return match[0]
}
