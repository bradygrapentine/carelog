...
export default function MedicationsScreen() {
  ...
  useEffect(() => {
    ...
    if (next?.medications?.[0]) {
      writeWatchData({
        nextMedication: { name: next.medications[0].drug_name, dueAt: next.scheduled_time },
      })
    }
  }, [scheduled])
  ...
  const administeredSet = new Set(
    ((todayLog as TodayLogEntry[]) ?? []).map((l) => `${l.medication_id}|${l.scheduled_time}`),
  )
          ...
          return (
            <View style={styles.row}>
              <View style={styles.info}>
                ...
                <Text style={styles.medDose}>{med.dosage} · {item.scheduled_time}</Text>
              </View>
              <TouchableOpacity
                style={[styles.btn, given && styles.givenBtn]}
                disabled={given || logMutation.isPending}
                onPress={() =>
                  logMutation.mutate({
                    org_id: orgId!,
