...
import { digestMinuteOffset, isoWeekStamp } from "@carelog/utils";
import * as Sentry from "@sentry/nextjs";

/** Idempotency key for the weekly digest. One email per org per ISO week. */
export function weeklyDigestDedupKey(orgId: string, week: string): string {
  return "weekly_digest:" + orgId + ":" + week;
}
...
export function digestHtml(opts: {
  ...
  const previewHtml = entries
    ...
    .map((e) => {
      ...
      const meta = [date, mood, e.flagged ? "flagged for doctor" : ""]
        .filter(Boolean)
        .join(" \u00b7 ");
      return (
        '<div style="padding:12px 0;border-bottom:1px solid #f5f5f5;">' +
        '<p style="font-size:14px;color:#111;margin:0 0 4px;line-height:1.5;">' +
        snippet +
        "</p>" +
  ...
  const moreHtml =
    moreCount > 0
      ? '<p style="font-size:13px;color:#aaa;margin:8px 0 0;">+ ' +
        moreCount +
        " more " +
        (moreCount === 1 ? "entry" : "entries") +
        "</p>"
      : "";
  ...
  const flaggedHtml =
    flaggedCount > 0
      ? '<div style="background:#eff6ff;border-left:3px solid #2563eb;padding:10px 14px;margin-bottom:20px;border-radius:0 6px 6px 0;">' +
        ...
        flaggedCount +
        (flaggedCount === 1 ? " entry was" : " entries were") +
  ...
  return (
    ...
    '<a href="' +
    journalUrl +
    '" style="display:inline-block;background:#111;color:#fff;font-size:14px;font-weight:500;padding:10px 20px;border-radius:8px;text-decoration:none;">View the full journal \u2192</a>' +
    "</div>" +
    '<hr style="border:none;border-top:1px solid #eee;margin:32px 0 16px;">' +
    "<p style=\"font-size:12px;color:#bbb;margin:0;\">You're receiving this because you're part of the " +
    orgName +
    " care team on CareSync.</p>" +
    "</body></html>"
  );
}
  ...
  async ({ step, logger }) => {
    try {
      ...
      const activeOrgs = await step.run("find-active-orgs", async () => {
        ...
        return Array.from(seen.entries()).map(([orgId, recipientId]) => ({
          orgId,
          recipientId,
        }));
      });
      ...
      await Promise.all(
        activeOrgs.map(({ orgId, recipientId }) =>
          step.run("send-digest-" + orgId, async () => {
            ...
            if (entriesError)
              throw new Error("Entries query failed: " + entriesError.message);
            ...
            if (membershipsError)
              throw new Error(
                "Memberships query failed: " + membershipsError.message,
              );
            ...
            const assigneeIds = [
              ...new Set((rawShifts ?? []).map((s) => s.assignee_user_id)),
            ];
            ...
            const digestShifts: Shift[] = (rawShifts ?? []).map((s) => ({
              ...
              end_at: s.end_at,
              assignee_name:
                assigneeNameMap.get(s.assignee_user_id) ?? "Care team member",
              status: s.status,
            }));
            ...
            await step.sleep(
              "stagger-" + orgId,
              digestMinuteOffset(orgId) + "s",
            );

            if (!resend) {
              logger.warn(
                "RESEND_API_KEY not set — skipping email for org " + orgId,
              );
              return;
            }

            // Idempotency: one digest per org per ISO week. INSERT the dedup row
            // BEFORE calling Resend so an Inngest retry (or a concurrent worker)
            // hits the unique constraint and skips instead of double-sending.
            const now = new Date();
            const dedupKey = weeklyDigestDedupKey(orgId, isoWeekStamp(now));
            ...
            if (insertError) {
              ...
              if (code === "23505") {
                logger.info(
                  "Weekly digest already dispatched or in-flight for org " +
                    orgId +
                    " — skipping",
                );
                return;
              }
              // Real DB error — fail fast so Inngest retries. PHI-clean tags only.
              Sentry.captureException(insertError, {
                tags: {
                  component: "inngest.weeklyDigest",
            ...
            try {
              ...
              // Leave the dedup row pending (sent_at null). Retries hit the unique
              // violation and skip; trade-off is one failed send = no digest this
              // week. PHI-clean tags only — no email body / addresses to Sentry.
            ...
            // Mark the row sent — it is now permanent for this org+week.
            await supabaseAdmin
              .from("email_dispatch_log")
            ...
            logger.info(
              "Digest sent to " + emails.length + " members of " + org.name,
            );
          }),
        ),
      );
