...
export function parseOcrText(rawText: string): {
  ...
  const instructionsMatch = rawText.match(/(?:take|daily|twice|once|with\s+food|with\s+water)[^\n.]*/i)
  const firstLine         = rawText.split('\n')[0]?.trim() ?? rawText.slice(0, 50)

  return {
    drug_name:    firstLine.replace(/\d+\.?\d*\s*(?:mg|mcg|ml|g|IU)/gi, '').trim() || 'Unknown',
...
export const ocrPrescription = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    await step.run('update-needs-review', async () => {
      ...
      await supabaseAdmin
        .from('ocr_jobs')
        .update({ status: 'needs_review', raw_text: rawText, parsed_payload: parsed })
        .eq('id', jobId)
    })
  }
)
