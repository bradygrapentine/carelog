...
export const burnoutRouter = router({
  ...
  orgSummary: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const byWeek = new Map<string, { sleep: number[], stress: number[], support: number[], count: number }>()
      for (const row of (data ?? [])) {
        const existing = byWeek.get(row.week_stamp) ?? { sleep: [], stress: [], support: [], count: 0 }
