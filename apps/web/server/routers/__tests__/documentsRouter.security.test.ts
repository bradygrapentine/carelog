...
function makeSelectChain(result: object) {
  const chain: any = { select: () => chain, eq: () => chain, not: () => chain, order: () => chain }
...
describe('documents.list — authentication', () => {
  it('throws UNAUTHORIZED when no user in context', async () => {
    await expect(anonCaller.documents.list({ org_id: ORG_ID, recipient_id: REC_ID }))
...
describe('documents.list — authorization', () => {
  const sampleDocs = [
    { id: DOC_ID, display_name: 'POA.pdf', doc_type: 'poa', file_size: 102400, uploaded_by: USER_ID, created_at: '2026-04-09T00:00:00Z' },
  ]
  ...
  it('returns documents for any authenticated org member', async () => {
    ...
    const result = await authedCaller.documents.list({ org_id: ORG_ID, recipient_id: REC_ID })
  ...
  it('throws FORBIDDEN when caller is not an org member', async () => {
    ...
    await expect(authedCaller.documents.list({ org_id: OTHER_ORG, recipient_id: REC_ID }))
  ...
  it('supporter role can list documents', async () => {
    ...
    vi.mocked(supabaseAdmin.from).mockImplementation(() => {
      callCount++
      if (callCount === 1) return makeSelectChain({ data: { role: 'supporter' }, error: null })
      return makeListChain({ data: sampleDocs, error: null })
    })
...
describe('documents.delete — authentication', () => {
  it('throws UNAUTHORIZED when no user in context', async () => {
    await expect(anonCaller.documents.delete({ id: DOC_ID, org_id: ORG_ID }))
      .rejects.toMatchObject({ code: 'UNAUTHORIZED' })
  })
})
...
describe('documents.delete — authorization', () => {
  it('throws FORBIDDEN when role is caregiver', async () => {
    ...
    await expect(authedCaller.documents.delete({ id: DOC_ID, org_id: ORG_ID }))
      .rejects.toMatchObject({ code: 'FORBIDDEN' })
  })
  ...
  it('throws FORBIDDEN when caller is not an org member', async () => {
    ...
    const OTHER_ORG = '99dc6d19-6712-4b26-8797-b4e544e01b99'
    await expect(authedCaller.documents.delete({ id: DOC_ID, org_id: OTHER_ORG }))
      .rejects.toMatchObject({ code: 'FORBIDDEN' })
  })
  ...
  it('throws NOT_FOUND when doc does not belong to org', async () => {
    ...
    vi.mocked(supabaseAdmin.from).mockImplementation(() => {
      ...
      return makeSelectChain({ data: null, error: { code: 'PGRST116', message: 'not found' } })
    })
  ...
  it('deletes storage object and DB row when coordinator', async () => {
    ...
    vi.mocked(supabaseAdmin.from).mockImplementation(() => {
      ...
      if (callCount === 2) return makeSelectChain({ data: { storage_path: storagePath }, error: null })
    ...
    vi.mocked(supabaseAdmin.storage.from).mockReturnValue({ remove: removeStub } as any)

    const result = await authedCaller.documents.delete({ id: DOC_ID, org_id: ORG_ID })
