...
const SLEEP_LABELS: Record<number, string>   = { 1: 'Very poor', 2: 'Poor',   3: 'Fair',     4: 'Good', 5: 'Great'        }
const STRESS_LABELS: Record<number, string>  = { 1: 'None',      2: 'Low',    3: 'Moderate', 4: 'High', 5: 'Overwhelming' }
const SUPPORT_LABELS: Record<number, string> = { 1: 'None',      2: 'Little', 3: 'Some',     4: 'Good', 5: 'Strong'       }
      ...
      <div className="px-4 py-4">
        {saved ? (
          <div className="py-4 text-center" role="status">
            <p className="text-sm font-medium text-emerald-700">Check-in saved.</p>
            <p className="text-xs text-gray-400 mt-1">We&apos;ll remind you next week.</p>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <fieldset>
              <div className="flex items-center justify-between mb-2">
                <legend className="text-xs font-medium text-gray-600">Sleep quality</legend>
                <div className="flex items-center gap-1.5" aria-hidden="true">
                  <span className={'text-sm font-semibold tabular-nums ' + goodColor(sleepScore)}>{sleepScore}/5</span>
                  <span className={'text-xs ' + goodColor(sleepScore)}>{SLEEP_LABELS[sleepScore]}</span>
                </div>
              </div>
              <input
                type="range"
                ...
                aria-valuetext={sleepScore + ' out of 5 — ' + SLEEP_LABELS[sleepScore]}
              />
            ...
            <fieldset>
              <div className="flex items-center justify-between mb-2">
                <legend className="text-xs font-medium text-gray-600">Stress level</legend>
                <div className="flex items-center gap-1.5" aria-hidden="true">
                  <span className={'text-sm font-semibold tabular-nums ' + stressColor(stressScore)}>{stressScore}/5</span>
                  <span className={'text-xs ' + stressColor(stressScore)}>{STRESS_LABELS[stressScore]}</span>
                </div>
              </div>
              <input
                type="range"
                ...
                aria-valuetext={stressScore + ' out of 5 — ' + STRESS_LABELS[stressScore]}
              />
              <div className="flex justify-between text-xs text-gray-300 mt-0.5" aria-hidden="true">
                <span>None</span>
            ...
            <fieldset>
              <div className="flex items-center justify-between mb-2">
                <legend className="text-xs font-medium text-gray-600">Support from others</legend>
                <div className="flex items-center gap-1.5" aria-hidden="true">
                  <span className={'text-sm font-semibold tabular-nums ' + goodColor(supportScore)}>{supportScore}/5</span>
                  <span className={'text-xs ' + goodColor(supportScore)}>{SUPPORT_LABELS[supportScore]}</span>
                </div>
              </div>
              <input
                type="range"
                ...
                aria-valuetext={supportScore + ' out of 5 — ' + SUPPORT_LABELS[supportScore]}
              />
              <div className="flex justify-between text-xs text-gray-300 mt-0.5" aria-hidden="true">
                <span>None</span>
                <span>Strong</span>
            ...
            {/* Notes */}
            <div>
              <label htmlFor="burnout-notes" className="block text-xs font-medium text-gray-600 mb-1">
                Anything you want to add? <span className="font-normal text-gray-400">(optional)</span>
              </label>
              <textarea
                id="burnout-notes"
                value={notes}
