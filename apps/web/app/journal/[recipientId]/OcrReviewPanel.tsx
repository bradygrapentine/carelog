...
export function OcrReviewPanel({ orgId, recipientId }: Props) {
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const res  = await authenticatedFetch('/api/ocr/upload', { method: 'POST', body: fd })
      const data = await res.json()
      if (data.jobId) {
  ...
  return (
    <div className="bg-white border border-gray-100 rounded-xl shadow-sm overflow-hidden">
      <div className="px-4 py-3 border-b border-gray-50 flex items-center justify-between">
        <p className="text-sm font-medium text-gray-700">Scan prescription label</p>
        <div className="flex items-center gap-2">
          {uploading && <span className="text-xs text-gray-400">Processing...</span>}
          {uploadMsg && !uploading && <span className="text-xs text-gray-500">{uploadMsg}</span>}
          <button
            type="button"
      ...
      <div className="px-4 py-3">
        ...
        {jobs.map(job => {
          const edit = edits[job.id] ?? { drug_name: '', dosage: '', instructions: '' }
          return (
            <div key={job.id} className="mb-4 pb-4 border-b border-gray-50 last:border-0">
              ...
              <div className="space-y-2">
                <div>
                  ...
                  <input
                    ...
                    onChange={e => setField(job.id, 'drug_name', e.target.value)}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={e => setField(job.id, 'instructions', e.target.value)}
                    className="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:border-gray-400"
                  />
                </div>
              </div>
