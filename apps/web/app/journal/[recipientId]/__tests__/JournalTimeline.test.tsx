...
describe('JournalTimeline — empty state', () => {
  it('shows caregiver prompt when canFlag is true', () => {
    render(<JournalTimeline events={[]} currentUserId="u1" canFlag={true} recipientId="r1" onFlag={vi.fn()} />)
  ...
  it('shows supporter message when canFlag is false', () => {
    render(<JournalTimeline events={[]} currentUserId="u1" canFlag={false} recipientId="r1" onFlag={vi.fn()} />)
...
describe('JournalTimeline — human journal entries', () => {
  ...
  it('navigates to detail page when a journal card is clicked', () => {
    render(<JournalTimeline events={[makeEvent({ id: 'evt-nav' })]} currentUserId="u1" canFlag={false} recipientId="r1" onFlag={vi.fn()} />)
    const card = screen.getByTestId('journal-entry')
    fireEvent.click(card)
    expect(window.location.href).toBe('/journal/r1/entry/evt-nav')
  })

  it('renders the entry text', () => {
    render(<JournalTimeline events={[makeEvent()]} currentUserId="u1" canFlag={false} recipientId="r1" onFlag={vi.fn()} />)
    expect(screen.getByText('Dad had a calm day.')).toBeInTheDocument()
  })

  it('renders mood badge when mood is set', () => {
    render(<JournalTimeline events={[makeEvent({ payload: { text: 'OK', mood: 'good' } })]} currentUserId="u1" canFlag={false} recipientId="r1" onFlag={vi.fn()} />)
    expect(screen.getByText('good')).toBeInTheDocument()
  })

  it('omits mood badge when mood is absent', () => {
    render(<JournalTimeline events={[makeEvent({ payload: { text: 'Note' } })]} currentUserId="u1" canFlag={false} recipientId="r1" onFlag={vi.fn()} />)
    expect(screen.queryByText('good')).not.toBeInTheDocument()
  })

  it('shows "Flag for doctor" button when canFlag is true', () => {
    render(<JournalTimeline events={[makeEvent()]} currentUserId="u1" canFlag={true} recipientId="r1" onFlag={vi.fn()} />)
  ...
  it('shows "Flagged for doctor" badge and Unflag button when entry is flagged', () => {
    render(<JournalTimeline events={[makeEvent({ flagged: true })]} currentUserId="u1" canFlag={true} recipientId="r1" onFlag={vi.fn()} />)
  ...
  it('calls onFlag with eventId and new flagged value when flag button is clicked', () => {
    const onFlag = vi.fn()
    render(<JournalTimeline events={[makeEvent({ id: 'evt-1', flagged: false })]} currentUserId="u1" canFlag={true} recipientId="r1" onFlag={onFlag} />)
...
describe('JournalTimeline — system events', () => {
  it('renders system events as compact display, not as journal card', () => {
    ...
    render(<JournalTimeline events={[sysEvent]} currentUserId="u1" canFlag={false} recipientId="r1" onFlag={vi.fn()} />)
  ...
  it('does not show flag button for system events', () => {
    ...
    render(<JournalTimeline events={[sysEvent]} currentUserId="u1" canFlag={true} recipientId="r1" onFlag={vi.fn()} />)
    expect(screen.queryByRole('button', { name: 'Flag for doctor' })).not.toBeInTheDocument()
  })
})
