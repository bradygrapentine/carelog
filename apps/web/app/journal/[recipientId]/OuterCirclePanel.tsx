...
export function OuterCirclePanel({ recipientId, orgId, currentUserRole }: Props) {
  const [expanded,     setExpanded]     = useState(false)
  const [showForm,     setShowForm]     = useState(false)
  const [title,        setTitle]        = useState('')
  const [description,  setDescription]  = useState('')
  const [requestType,  setRequestType]  = useState<'meal' | 'transport' | 'errand' | 'visit' | 'other'>('meal')
  ...
  return (
    <div className="bg-white border border-gray-100 rounded-xl shadow-sm overflow-hidden">
      <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
        <p className="text-sm font-medium text-gray-700">Volunteer requests</p>
        <button
          type="button"
      ...
      <div className="px-4 py-4">
        {requests.filter(r => r.active).length > 0 && (
          <div className="mb-4 space-y-3">
            {requests.filter(r => r.active).map(req => {
        ...
        {!showForm ? (
          ...
          <form onSubmit={handleCreate} className="space-y-3">
            <div>
              <label htmlFor="oc-title" className="block text-xs text-gray-500 mb-1">Title</label>
              <input
                id="oc-title"
                type="text"
                required
            ...
            <div>
              <label htmlFor="oc-description" className="block text-xs text-gray-500 mb-1">Description (optional)</label>
              <textarea
                id="oc-description"
                maxLength={1000}
                value={description}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label htmlFor="oc-type" className="block text-xs text-gray-500 mb-1">Type</label>
                <select
                  id="oc-type"
                  value={requestType}
              ...
              <div>
                <label htmlFor="oc-slots" className="block text-xs text-gray-500 mb-1">Slots needed</label>
                <select
                  id="oc-slots"
                  value={slotsTotal}
            ...
            <div>
              <label htmlFor="oc-needed-by" className="block text-xs text-gray-500 mb-1">Needed by (optional)</label>
              <input
                id="oc-needed-by"
                type="date"
                value={neededBy}
