    ...
    <Document>
      <Page size="LETTER" style={s.page}>
        ...
        <View style={s.section}>
          <Text style={s.sectionHead}>
            {"Journal Entries (" +
              data.care_events.filter((e) => e.entry_kind === "human").length +
              ")"}
          </Text>
          {data.care_events.filter((e) => e.entry_kind === "human").length ===
            0 && <Text style={s.value}>No journal entries on record.</Text>}
          {data.care_events
            .filter((e) => e.entry_kind === "human")
            .map((e, i) => (
              <View key={i} style={s.item}>
                <View style={s.row}>
                ...
                {e.payload?.mood ? (
                  <View style={s.row}>
                    <Text style={s.label}>Mood</Text>
                    <Text style={s.value}>{String(e.payload.mood)}</Text>
                  </View>
                ) : null}
                {e.flagged && (
                  <Text
        ...
        <Text style={s.footer}>
          {"Generated by CareSync · " + data.exported_at}
        </Text>
      </Page>
    </Document>
  );
