...
describe("POST /api/brief", () => {
  ...
  it("returns 400 when org_id is not a UUID", async () => {
    const res = await POST(
      briefRequest({ org_id: "not-a-uuid", recipient_id: VALID_RECIP_ID }),
    );
  ...
  it("returns 400 when recipient_id is not a UUID", async () => {
    const res = await POST(
      briefRequest({ org_id: VALID_ORG_ID, recipient_id: "bad-id" }),
    );
    expect(res.status).toBe(400);
  });
  ...
  it("returns 403 when caller is not a coordinator", async () => {
    vi.mocked(supabaseAdmin.from).mockImplementationOnce(
      () =>
        makeSelectChain({
          data: { role: "supporter", accepted_at: "2026-01-01" },
          error: null,
        }) as any,
    );
  ...
  it("returns 200 with { share_token, id } on success", async () => {
    vi.mocked(supabaseAdmin.from)
      ...
      // previousBrief lookup — null means this is the first brief, so the
      // generator falls back to the last 24h window.
      .mockImplementationOnce(
      ...
      .mockImplementationOnce(
        () =>
          makeSelectChain({
            data: { id: BRIEF_ID, share_token: "abc123" },
            error: null,
          }) as any,
      );
