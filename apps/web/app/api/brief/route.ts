...
export async function POST(request: NextRequest) {
  try {
    ...
    // Fetch recent journal entries (last 30 days, human, journal, limit 10)
    ...
    const content = {
      ...
      medications:    (medications ?? []).map((m: { drug_name: string; dosage: string | null; instructions: string | null }) => ({
        drug_name:    m.drug_name,
        dosage:       m.dosage,
        instructions: m.instructions,
      })),
      recent_entries: (entries ?? []).map((e: { occurred_at: string; payload: { text?: string; mood?: string } | null; flagged: boolean }) => ({
