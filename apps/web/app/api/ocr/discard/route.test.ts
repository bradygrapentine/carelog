...
function postRequest(body: unknown) {
  return new NextRequest("http://localhost/api/ocr/discard", {
    method: "POST",
...
describe("POST /api/ocr/discard", () => {
  ...
  it("returns 400 when jobId missing", async () => {
    const res = await POST(postRequest({ orgId: VALID_ORG_ID }));
  ...
  it("returns 500 when DB update fails", async () => {
    vi.mocked(supabaseAdmin.from)
      ...
      // job lookup — status read for state machine
      .mockImplementationOnce(
        () =>
          makeSelectChain({
            data: { id: VALID_JOB_ID, status: "needs_review" },
            error: null,
          }) as any,
      )
      // update fails
  ...
  it("returns 200 { ok: true } on success", async () => {
    vi.mocked(supabaseAdmin.from)
      ...
      // update succeeds — optimistic lock: count: 1 indicates row was updated
      .mockImplementationOnce(
        () =>
