...
function makeStorageBucket(uploadError: unknown = null) {
  return {
    ...
    getPublicUrl: vi.fn().mockReturnValue({ data: { publicUrl: 'http://storage/img.png' } }),
  }
}
...
function uploadRequest(orgId = ORG_ID, recipientId = REC_ID, includeFile = true) {
...
describe('POST /api/ocr/upload', () => {
  ...
  it('returns 403 when recipient does not belong to org', async () => {
    vi.mocked(supabaseAdmin.from)
      .mockReturnValueOnce(makeSelectChain({ data: { role: 'coordinator' }, error: null }) as any)
  ...
  it('returns 500 when storage upload fails', async () => {
    vi.mocked(supabaseAdmin.from)
      .mockReturnValueOnce(makeSelectChain({ data: { role: 'coordinator' }, error: null }) as any)
      .mockReturnValueOnce(makeSelectChain({ data: { id: REC_ID }, error: null }) as any)
  ...
  it('returns 200 with jobId on success', async () => {
    vi.mocked(supabaseAdmin.from)
      .mockReturnValueOnce(makeSelectChain({ data: { role: 'coordinator' }, error: null }) as any)
      .mockReturnValueOnce(makeSelectChain({ data: { id: REC_ID }, error: null }) as any)
      .mockReturnValueOnce(makeInsertChain({ data: { id: JOB_ID }, error: null }) as any)
    vi.mocked(supabaseAdmin.storage.from).mockReturnValue(makeStorageBucket() as any)
