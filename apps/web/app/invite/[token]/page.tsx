  ...
  useEffect(() => {
    fetch('/api/invite/' + token)
      ...
      .then(data => {
        ...
        else { setEmail(data.email); setOrgName(data.orgName); setRole(data.role); setStatus('ready') }
      })
...
async function handleAccept() {
  ...
  if (user.email?.toLowerCase().trim() !== email.toLowerCase().trim()) {
    setError(
      'You are signed in as ' + user.email + ' but this invite was sent to ' +
      email + '. Please sign out and sign in with ' + email + ' to accept.'
    )
    setStatus('error')
    return
  }
  ...
  const roleLabels: Record<string, string> = {
    coordinator: 'Coordinator', caregiver: 'Caregiver',
    supporter: 'Supporter', aide: 'Professional aide',
  }
  ...
  if (status === 'done') return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
      <div className="bg-white border border-gray-100 rounded-xl p-8 shadow-sm max-w-md w-full text-center">
        <div className="w-12 h-12 bg-green-50 rounded-full flex items-center justify-center mx-auto mb-4">
          <svg className="w-6 h-6 text-green-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
          </svg>
        </div>
        <h1 className="text-xl font-semibold text-gray-900 mb-2">You have joined the team</h1>
  ...
  if (status === 'error') return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
      <div className="bg-white border border-gray-100 rounded-xl p-8 shadow-sm max-w-md w-full text-center">
        ...
        <a href="/signin" className="inline-block px-4 py-2 bg-gray-900 text-white text-sm font-medium rounded-lg">Go to Carelog</a>
      </div>
    </div>
  )
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
      <div className="bg-white border border-gray-100 rounded-xl p-8 shadow-sm max-w-md w-full">
        <h1 className="text-xl font-semibold text-gray-900 mb-2">You have been invited to join a care team</h1>
        ...
        <button
          ...
          className="w-full py-2.5 px-4 bg-gray-900 text-white text-sm font-medium rounded-lg hover:bg-gray-800 disabled:opacity-50 transition-colors"
        >
          {status === 'accepting' ? 'Joining...' : 'Accept invitation'}
        </button>
        <p className="text-center text-xs text-gray-400 mt-4">You will need to sign in or create an account to continue.</p>
      </div>
    </div>
  )
}