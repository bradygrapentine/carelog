...
describe("MedicationChecklist — disabled states", () => {
  ...
  it("both buttons are disabled for supporter regardless of log state", () => {
    render(
      <MedicationChecklist {...defaultProps} currentUserRole="supporter" />,
    );
    expect(screen.getByRole("button", { name: /gave it/i })).toBeDisabled();
    expect(screen.getByRole("button", { name: /missed/i })).toBeDisabled();
  });
...
describe("MedicationChecklist — data-testid selectors", () => {
  it("renders medication-checklist-item for each scheduled medication", () => {
    render(<MedicationChecklist {...defaultProps} />);
    expect(
      document.querySelectorAll('[data-testid="medication-checklist-item"]'),
    ).toHaveLength(sampleSchedules.length);
  });

  it("does NOT render dose-given-indicator when medication is not logged", () => {
    render(<MedicationChecklist {...defaultProps} />);
    expect(
      document.querySelector('[data-testid="dose-given-indicator"]'),
    ).toBeNull();
  });

  it("renders dose-given-indicator when isLogged=true (medication already logged)", () => {
    vi.mocked(trpc.medications.todayLog.useQuery).mockReturnValue({
      data: [
        { medication_id: "med-1", scheduled_time: "08:00:00", action: "given" },
      ],
