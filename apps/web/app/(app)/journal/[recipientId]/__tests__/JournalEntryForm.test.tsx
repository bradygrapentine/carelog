...
function setup(overrides: Partial<React.ComponentProps<typeof JournalEntryForm>> = {}) {
...
describe('JournalEntryForm', () => {
  it('renders a collapsed textarea with the correct placeholder', () => {
    setup()
    expect(screen.getByPlaceholderText('Share how today went...')).toBeInTheDocument()
  ...
  it('hides prompt suggestions once the user types text', () => {
    ...
    expect(screen.queryByText('Need a starting point?')).not.toBeInTheDocument()
  })
  ...
  it('deselects a mood when clicked twice', () => {
    ...
    fireEvent.click(goodBtn)
    fireEvent.click(goodBtn)
    expect(goodBtn).not.toHaveClass('bg-green-100')
  })
  ...
  it('switches mood when a different mood is clicked', () => {
    ...
    expect(screen.getByText('Okay')).toHaveClass('bg-yellow-100')
    expect(screen.getByText('Good')).not.toHaveClass('bg-green-100')
  })
  ...
  it('calls onPost with trimmed text and selected mood on submit', async () => {
    ...
    fireEvent.focus(textarea)
    fireEvent.change(textarea, { target: { value: '  Dad was calm today.  ' } })
    fireEvent.click(screen.getByText('Good'))
